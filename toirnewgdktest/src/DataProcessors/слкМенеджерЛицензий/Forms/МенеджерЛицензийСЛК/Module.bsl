...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СохраненыПерсональныеНастройки = Ложь;
...
Процедура СписокКлючей(Команда)
	...
	Иначе
		...
		Для Ин = 0 По СписокКлючей.ВГраница() Цикл
			СписокКлючейСтрокой = СписокКлючейСтрокой + Символы.ПС + Символы.ПС+ "Номер ключа: " + СписокКлючей[Ин].KeyNo + ", рег.номер: " + СписокКлючей[Ин].RegNo + 
								  ", кол-во лиценций: " + СписокКлючей[Ин].LicenceCount + ", дата активации: " + Формат(СписокКлючей[Ин].ActivationDate, "ДФ=dd.MM.yyyy");
		КонецЦикла;
		
		ПоказатьПредупреждение(,"Список установленных на сервере СЛК ключей серии " + ТекущаяСерия.Серия + ": " + СписокКлючейСтрокой);
	КонецЕсли;
...
Процедура РегистрационныеНомераКлючей(Команда)
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		слкМенеджерЗащиты.СообщитьОбОшибкеСЛК(ОписаниеОшибки, ТекущаяСерия.Серия);
	Иначе
		ПоказатьПредупреждение(,"Список регистрационных номеров установленных на сервере СЛК ключей серии " + ТекущаяСерия.Серия + ": " + 
								Символы.ПС + РегистрационныеНомераКлючей);
	КонецЕсли;
