...
&НаКлиенте
Процедура ДекорацияСмТакжеОбработкаНавигационнойСсылки(
		Элемент,
		НавигационнаяСсылкаФорматированнойСтроки,
		СтандартнаяОбработка)
...
Процедура ОтправитьСообщениеВТехПоддержку(ТекстСообщения)
	...
	ДанныеСообщения.Вставить("Тема",       НСтр("ru = 'Интернет-поддержка. Монитор Портала 1С:ИТС'"));
...
Функция МаркированныйСписокСПериодами(ДанныеСписка, СписокСервисов1С)
	...
	Для ИндексЭлемента = 0 По СписокДоговоров.ВГраница() Цикл
		...
		ЧастиСтроки.Добавить(
			?(ИндексЭлемента > 0, "<br />", "")
			+ "● "
			+ НормализованнаяСтрокаXML(ЭлементСписка.name)
			+ " ");
...
Функция МаркированныйФорматированныйСписокСсылокСмТакже(ТаблицаСсылок, НомерКолонки)
	...
	Для Итератор = 0 По СтрокиТаблицы.ВГраница() Цикл
		...
		ЧастиСтроки.Добавить(
			...
			+ НормализованнаяСтрокаXML(СтрокаТаблицы.Заголовок)
			+ "</a>");
...
Процедура ОтобразитьСостояниеОшибка(
		...
		ПоказатьПоляАутентификации = Ложь)
...
Функция ТекстИсправления(ОбновлениеКонфигурации, Исправления)
	...
	Иначе
		ПолеПоиска            = ?(ДляНовойВерсии, "ДляНовойВерсии", "ДляТекущейВерсии");
		КоличествоИсправлений = Исправления.НайтиСтроки(Новый Структура(ПолеПоиска, Истина)).Количество();
	КонецЕсли;
