...
Функция ДобавитьКлассификатор(СтрокаXMLИлиФайл, ПервичныйИсточник = "", MIMEТип = "", УникальныйИдентификатор = Неопределено) Экспорт
	...
	Исключение
		#Если Не ВнешнееСоединение Тогда
			ШаблонСообщения = НСтр("ru = 'Файл (%1) поврежден или имеет неверный формат.'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрокаXMLИлиФайл.Имя);
...
Функция ПроверитьРазархивироватьФайл(Файл, ТипЧитателя, ПервичныйИсточник, MIMEТип = "",УникальныйИдентификатор = Неопределено)
	...
	Если НРег(Файл.Расширение) = ".aml" Тогда
		...
		Исключение
			#Если Не ВнешнееСоединение Тогда
				ШаблонСообщения = НСтр("ru = 'Файл (%1) не удалось распаковать: файл поврежден, имеет неверный формат, или не достаточно места на диске.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Файл.Имя);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			#КонецЕсли
			Возврат Неопределено
		КонецПопытки;
	...
	ИначеЕсли НРег(Файл.Расширение) = ".zip"
		...
		Для Каждого Элемент Из Zip.Элементы Цикл
			...
			Исключение
				#Если Не ВнешнееСоединение Тогда
					ШаблонСообщения = НСтр("ru = 'Файл (%1) не удалось извлечь из (%2): архив поврежден, имеет неверный формат, или не достаточно места на диске.'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Элемент.Имя, Файл.Имя);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
				#КонецЕсли
				Продолжить
			КонецПопытки;
