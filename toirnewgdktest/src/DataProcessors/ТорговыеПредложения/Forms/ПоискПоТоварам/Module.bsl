...
Процедура ЗаполнитьПараметрыЗапроса(ПараметрыЗапроса, Отказ)
	...
	Если ЗначениеЗаполнено(АдресКорзины) Тогда
	...
	Если Не Отказ И Результат <> Неопределено И Результат.Статус = "Выполнено" Тогда
		...
		Если ЗначениеЗаполнено(Результат.АдресРезультата) 
			...
			Если Контрагенты.Количество() = 0 Тогда
				...
				ТребуетсяПовторноеПодключениеОрганизации = 
					ИнтеграцияТорговыеПредложенияКлиент.ОрганизацияНеПодключенаТребуетсяПовторноеПодключение(
						Организация);
				
				Если ТребуетсяПовторноеПодключениеОрганизации Тогда
					ОповещениеПослеПодключенияОрганизации = 
						Новый ОписаниеОповещения("ПодключениеОрганизацииПродолжение", ЭтотОбъект);
					ИнтеграцияТорговыеПредложенияКлиент.ОткрытьФормуПодключенияОрганизацииСПроверкойПодключения(
						Организация, ЭтотОбъект, ОповещениеПослеПодключенияОрганизации);
				Иначе
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Результат.ДанныеКорзины) Тогда
			ТорговыеПредложенияСлужебный.ПеречитатьКорзинуНаФормахПоискаПоОтборуИПоТоварам(
				АдресКорзины, АдресКомандыДобавитьТовар, Результат.ДанныеКорзины, ТоварыКорзины);
		КонецЕсли;
...
Процедура ОткрытьФормуОформленияЗаказов(РежимЗапросаЦен = Ложь)
	...
	Для Каждого СтрокаСписка Из ВыбранныеСтроки Цикл
		...
		ИдентификаторПредложения = СтрокаСписка["Идентификатор_" + ИдентификаторКонтрагента];
		Если ПустаяСтрока(ИдентификаторПредложения) Тогда
			Продолжить;
		КонецЕсли;
		
		ОтборСтрок = Новый Структура("Идентификатор", ИдентификаторПредложения);
		СтрокаДанных = ТоварыКорзины.НайтиСтроки(ОтборСтрок);
		Если ЗначениеЗаполнено(СтрокаДанных) Тогда
			КоличествоКорзина = СтрокаДанных[0].Количество;
		КонецЕсли;
		
		КоличествоВыбрано = СтрокаСписка.Количество;
		Количество = КоличествоВыбрано + КоличествоКорзина;
		
		ДобавитьТоварИзменитьКоличество(ИдентификаторПредложения, Количество);
		
		ОткрытьФормуКорзины = Истина;
		
	КонецЦикла;
	
	Если ОткрытьФормуКорзины Тогда
		ОчиститьСообщения();
...
Процедура ДобавитьТоварИзменитьКоличество(ИдентификаторПредложения, Количество)
	...
	Если Не ЗначениеЗаполнено(АдресКорзины) Тогда
		...
		СоздатьКорзину(Организация, АдресКорзины, АдресКомандыДобавитьТовар);
	...
	КомандыТорговогоПредложения = ТорговыеПредложенияКлиент.КомандыТорговогоПредложения(
		ИдентификаторПредложения, ТоварыКорзины);
...
Процедура ПослеВыполненияДействияНадТоваром(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыбранныеСтроки[0].Пометка = Ложь;
	
	ТорговыеПредложенияКлиент.ОповеститьОбИзмененииКорзины(Результат.АдресРезультата, ЭтотОбъект);
	Если Список.НайтиСтроки(Новый Структура("Пометка", Истина)).Количество() = 0 Тогда
		Закрыть();
	КонецЕсли;
