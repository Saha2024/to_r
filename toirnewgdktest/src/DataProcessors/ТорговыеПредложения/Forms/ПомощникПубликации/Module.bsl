...
&НаКлиенте
Процедура ПодключенныеОрганизацииПриИзменении(Элемент)
	Значение = Не ОрганизацияОтбораФормыПоиска.Пустая();
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ОткрытьМоиТорговыеПредложения", "Доступность", Значение);
КонецПроцедуры
...
Процедура ОткрытьМоиТорговыеПредложенияНажатие(Элемент)
	...
	Оповестить("ТорговыеПредложения_ПоискПоОтборам_Обновить", ПараметрыОткрытияФормы);
...
#Область СтатистикаПубликации
	...
	Если ПараметрыВыполнения.ИспользоватьСтатистикуПубликации Тогда
		...
		ИначеЕсли КоличествоОшибок = 0 Тогда
			...
			ТекстУспеха = СтрШаблон(НСтр("ru = 'Опубликовано: %1'"), КоличествоОпубликованных);
			ЗначениеУспеха = Новый ФорматированнаяСтрока(ТекстУспеха, , ЦветаСтиля.РезультатУспехЦвет);
			ЧастиЗаголовка.Добавить(ЗначениеУспеха);
...
#Область РегистрацияОрганизаций
	...
	Если ПараметрыВыполнения.ИспользоватьРегистрациюОрганизации Тогда
		...
		ПодключенныеОрганизации = 
			?(Статистика.Свойство("ПодключенныеОрганизации"), Статистика.ПодключенныеОрганизации, Новый Массив);
		...
		Если КоличествоОрганизаций = 1 Тогда
			ОрганизацияОтбораФормыПоиска = ПодключенныеОрганизации[0];
		КонецЕсли;
		Элементы.ОрганизацияОтбораФормыПоиска.СписокВыбора.ЗагрузитьЗначения(ПодключенныеОрганизации);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ОрганизацияОтбораФормыПоиска", "Видимость", КоличествоОрганизаций > 1);
