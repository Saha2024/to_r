...
Процедура ПроверитьАдресПоказатьПодсказку()
	...
	// ТОИР++
	// Исправление ошибки: если не подключена интернет-поддержка, то в структуре
	// СведенияОбАдресе.Идентификаторы нет используемых ниже полей, и падает ошибка.
	Если НЕ СведенияОбАдресе.Идентификаторы.Свойство("Регион") Тогда
		СведенияОбАдресе.Идентификаторы.Вставить("Регион", "");
	КонецЕсли;
	Если НЕ СведенияОбАдресе.Идентификаторы.Свойство("Район") Тогда
		СведенияОбАдресе.Идентификаторы.Вставить("Район", "");
	КонецЕсли;
	Если НЕ СведенияОбАдресе.Идентификаторы.Свойство("МуниципальныйРайон") Тогда
		СведенияОбАдресе.Идентификаторы.Вставить("МуниципальныйРайон", "");
	КонецЕсли;
	Если НЕ СведенияОбАдресе.Идентификаторы.Свойство("Город") Тогда
		СведенияОбАдресе.Идентификаторы.Вставить("Город", "");
	КонецЕсли;
	// ТОИР--
...
Процедура ЗаполнитьАдресаЗаказа(Форма, ТекущиеДанные)
	...
	Если СпособДоставки = "Самовывоз" Тогда
		Для каждого ТекущийАдрес Из ТекущиеДанные.АдресаСамовывоза Цикл
			Элементы.АдресДоставки.СписокВыбора.Добавить(ТекущийАдрес.ЗначенияПолей, ТекущийАдрес.ПредставлениеАдреса);
		КонецЦикла;
	Иначе
...
Функция ПараметрыСозданияЗаказа()
	...
	Если ТекущиеДанные.СпособДоставки = "Самовывоз" Тогда
		СтрокиДанных = ТекущиеДанные.АдресаСамовывоза.НайтиСтроки(
			Новый Структура("ЗначенияПолей", ТекущиеДанные.АдресДоставки));
	Иначе
