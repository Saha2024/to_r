...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполненныеПараметрыЗапроса = Новый Структура;
	ЗаполненныеПараметрыЗапроса.Вставить("ПолучитьДеревоКатегорий", ПолучитьДеревоКатегорий);
	ЗаполненныеПараметрыЗапроса.Вставить("ПолучитьДанныеКорзины", ЗначениеЗаполнено(АдресКорзины));
	НайтиТорговыеПредложенияВФоне(ЗаполненныеПараметрыЗапроса, Ложь);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ТорговыеПредложения_ПоискПоОтборам_Обновить" Тогда
		...
		Если Параметр.Свойство("Контрагент") 
			...
			ЗначениеОтбора = ИННКонтрагента;
			Если ПустаяСтрока(ЗначениеОтбора) Тогда
				ЗначениеОтбора = НаименованиеКонтрагента;
			КонецЕсли;
			
			СтрокиДереваОтборов = ДеревоОтборов.ПолучитьЭлементы();
			НовыйОтбор = СтрокиДереваОтборов[0];
			НовыйОтбор.ЗначениеОтбора      = ЗначениеОтбора;
			НовыйОтбор.ПредставлениеОтбора = ЗначениеОтбора;
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	ПодключитьОбработчикОжидания("ПоказатьКартинкуПредложения", 0.5, Истина);
...
Процедура КатегорииПриАктивизацииСтроки(Элемент)
	...
	ПодключитьОбработчикОжидания("Подключаемый_ПриАктивизацииСтрокиКатегорииПродолжение", 0.5, Истина);
...
Процедура ОбработатьРезультатИзмененияФиксированногоОтбора(ТекущаяСтрокаДерева, ДанныеОтбораИзменены, Знач Результат)
	...
	Если Идентификатор = "Поставщик" Тогда
		...
		Иначе
			...
			Если ТипЗнч(Результат) <> Тип("Булево") Тогда
				// При выборе значения отбора из справочника Контрагенты, 
				// в результат возвращается тип ОпределяемыйТип.КонтрагентБЭД.
				...
				ТекущаяСтрокаДерева.ЗначениеОтбора       = ИННКонтрагента;
			КонецЕсли;
	...
	ИначеЕсли Идентификатор = "СИзображением" 
		Или Идентификатор = "ВНаличии" 
		Или Идентификатор = "СоСкидкой" Тогда
...
Процедура ОтборПоПоставщику(Команда)
	...
	НаименованиеКонтрагента = ТекущиеДанные.ИННКонтрагента;
...
Процедура ЗаполнитьОрганизации()
	...
	Организации.ЗагрузитьЗначения(БизнесСеть.ЗарегистрированныеОрганизации());
...
Процедура НайтиТорговыеПредложения(ПолучитьДеревоКатегорий = Ложь, СледующаяСтраница = Ложь)
	...
	ЗаполненныеПараметрыЗапроса = ЗаполнитьПараметрыЗапроса(Истина, ПолучитьДеревоКатегорий, Истина);
...
Процедура НайтиТорговыеПредложенияВФоне(
	...
	Если ЗаполненныеПараметрыЗапроса.ПолучитьДанныеКорзины Тогда
	...
	Элементы.СписокНаименованиеКонтрагента.Видимость = НЕ ЗначениеЗаполнено(Контрагент)
		И НЕ ЗначениеЗаполнено(ИдентификаторКонтрагента) И НЕ ЗначениеЗаполнено(НаименованиеКонтрагента);
	
	Если ФоновоеЗаданиеПоиска <> Неопределено 
		И ЗначениеЗаполнено(ФоновоеЗаданиеПоиска.ИдентификаторЗадания) Тогда
		ИнтеграцияБСПБЭД.ОтменитьВыполнениеЗадания(ФоновоеЗаданиеПоиска.ИдентификаторЗадания);
...
Процедура ПодключениеОрганизацииПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		Организации.Удалить(Организации.НайтиПоЗначению(ОтборОрганизация));
...
Функция ЗаполнитьПараметрыЗапроса(
	ОбновлятьЭлементыОтбора = Истина, ПолучитьДеревоКатегорий = Ложь, ПолучитьДанныеКорзины = Ложь)
	...
	ПараметрыЗапроса.Вставить("ПолучитьДеревоКатегорий", ПолучитьДеревоКатегорий);
	ПараметрыЗапроса.Вставить("ПолучитьДанныеКорзины", ПолучитьДанныеКорзины);
...
Процедура ДозаполнитьПараметрыЗапроса(ПараметрыЗапроса, Знач СледующаяСтраница = Ложь)
	...
	ДобавитьКонтекстныеБыстрыеОтборы();
...
Функция ОбработатьРезультатПоиска(АдресРезультата)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Результат, "Список") 
		...
		Для Каждого ЭлементКоллекции Из Результат.Список Цикл
			...
			ЗаполнитьПерсональнуюЦену(СтруктураПредложений, ЭлементКоллекции);
...
&НаСервере
Процедура ЗаполнитьПерсональнуюЦену(СтруктураПредложений, ЭлементКоллекции)
...
Процедура ЗаполнитьТаблицуТорговыхПредложений(Знач АдресРезультата)
	...
	Если ЗначениеЗаполнено(ДанныеПоискаИзСервиса.ДанныеКорзины) Тогда
		...
		Если НовыйДанныеКорзиныХэшСумма <> ДанныеКорзиныХэшСумма Тогда
			...
			ТорговыеПредложенияСлужебный.ПеречитатьКорзинуНаФормахПоискаПоОтборуИПоТоварам(
...
Процедура УстановитьЗаголовкиПодсказкиКолонокСЦенами(Элементы, Валюта, ОтображатьЦеныБезНДС)
	...
	Элементы.СписокПредставлениеЦены.Заголовок = 
		СтрШаблон(НСтр("ru = 'Цена %1, %2'"), ПредставлениеСтатусаНДС, Валюта);
	...
	Элементы.СписокПредставлениеЦены.Подсказка = СтрШаблон(ШаблонПодсказки, ПредставлениеСтатусаНДС, Валюта);
	
	Элементы.СписокЦенаДоСкидки.Заголовок = 
		СтрШаблон(НСтр("ru = 'Цена до скидки %1, %2'"), ПредставлениеСтатусаНДС, Валюта);
...
Процедура УстановитьВидимостьДоступность(Знач Форма, ПолучитьДеревоКатегорий = Ложь)
	...
	ИдентификаторКатегории = Форма.ИдентификаторКатегории;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		Не ПустаяСтрока(ИдентификаторЗаданияОбработкиФильтровРубрикатора) 
			Или Не ПустаяСтрока(ИдентификаторКатегории));
...
Процедура ЗаполнитьФиксированныеОтборыДерева()
	...
	Если Элементы.ОтборПоКонтрагенту.Видимость Тогда
		...
		Если ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
			НовыйОтбор.ЗначениеОтбора            = ИННКонтрагента;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьОтборПоКонтрагенту(ПараметрКонтрагент, Форма)
	...
	Если ТипЗнч(ПараметрКонтрагент) = Тип("Структура") Тогда
		...
		ПараметрКонтрагент.Свойство("Идентификатор", Форма.ИдентификаторКонтрагента);
...
&НаСервере
Процедура ДобавитьКонтекстныеБыстрыеОтборы()
	
	Если ОтборАртикулы.Количество() Тогда
		ПредставлениеОтбора = НСтр("ru = 'Артикул:'") + " " + СтрСоединить(ОтборАртикулы.ВыгрузитьЗначения(), ", ");
		БыстрыеОтборы.Добавить("Артикул", ПредставлениеОтбора);
	КонецЕсли;
	
	Если ОтборШтрихкоды.Количество() Тогда
		ПредставлениеОтбора = НСтр("ru = 'Штрихкод:'") + " " + СтрСоединить(ОтборШтрихкоды.ВыгрузитьЗначения(), ", ");
...
Процедура ПоказатьКартинкуПредложения()
	...
	Если Не Элементы.ПоказатьИзображения.Пометка Тогда
		Возврат;
	КонецЕсли;
	...
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено 
		Или ТекущиеДанные.АдресаКартинок.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьКартинкуПредложенияПродолжение()
	...
	СписокИдентификаторовРодителей = 
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.ИдентификаторКатегории);
	
	ПолучитьРодителейКатегории(Категории, СписокИдентификаторовРодителей, ТекущиеДанные.ИдентификаторКатегории);
	
	ИдентификаторВзрослойКатегории = "12207";
	Если Не ЗначениеЗаполнено(СписокИдентификаторовРодителей.Найти(ИдентификаторВзрослойКатегории)) Тогда
...
&НаКлиенте
Процедура ПолучитьРодителейКатегории(Дерево, СписокИдентификаторовРодителей, Знач ИдентификаторКатегорииПоиска)
	
	КоллекцияЭлементовДерева = Дерево.ПолучитьЭлементы();
	Для Каждого ЭлементДерева Из КоллекцияЭлементовДерева Цикл
		
		Если ЭлементДерева.Идентификатор = ИдентификаторКатегорииПоиска Тогда
			ИдентификаторРодителя = ЭлементДерева.ИдентификаторРодителя;
			Если Не ПустаяСтрока(ИдентификаторРодителя) Тогда
				СписокИдентификаторовРодителей.Добавить(ИдентификаторРодителя);
				ПолучитьРодителейКатегории(Категории, СписокИдентификаторовРодителей, ИдентификаторРодителя);
			КонецЕсли;
			
		Иначе
			ПолучитьРодителейКатегории(ЭлементДерева, СписокИдентификаторовРодителей, ИдентификаторКатегорииПоиска);
		КонецЕсли;
...
Процедура ОбработатьПараметрыФормы()
	...
	Если ЗначениеЗаполнено(ПараметрКонтрагент) Тогда
		ЗаполнитьОтборПоКонтрагенту(ПараметрКонтрагент, ЭтотОбъект);
	КонецЕсли;
	
	РазрешитьИзменениеКонтрагента = Не Параметры.ФиксированныйОтборПоКонтрагенту 
									Или Не ЗначениеЗаполнено(ПараметрКонтрагент);
	...
	ИначеЕсли Параметры.Свойство("ОтборШтрихКоды") И ЗначениеЗаполнено(Параметры.ОтборШтрихКоды) Тогда
...
&НаКлиенте
Процедура Подключаемый_ПриАктивизацииСтрокиКатегорииПродолжение()
	...
	Если Не (ИнтернетПоддержкаПодключена) 
		Или ПустаяСтрока(ИдентификаторКатегории) Тогда
		Возврат;
	КонецЕсли;
