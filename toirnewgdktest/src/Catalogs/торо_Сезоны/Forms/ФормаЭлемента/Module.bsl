...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Некорректная дата начала сезона.'");
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Некорректная дата окончания сезона.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,,Отказ);
	КонецПопытки;
	...
	Если ПарамДатаНачалаСезона > ПарамДатаОкончанияСезона Тогда
		Если Не ПроцессЗаписи Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗаписьюЗавершение",ЭтотОбъект,
							...
							НСтр("ru = 'При окончании года сезон не будет завершен. Сезон должен продолжаться в следующем году?'"),
