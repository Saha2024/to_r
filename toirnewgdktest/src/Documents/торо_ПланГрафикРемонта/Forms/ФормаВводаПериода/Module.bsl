...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОграниченияОтборов = ПолучитьОграниченияПоОтборам(ПланГрафикРемонта);
	...
	ДатаНачалаПланирования = ПланГрафикРемонта.ДатаПланирования;
	Если ЗначениеЗаполнено(ПланГрафикРемонта.ПериодичностьДетализации) Тогда		
		ДатаОкончанияПланирования = ОбщегоНазначенияУТКлиентСервер.РассчитатьДатуОкончанияПериода(ПланГрафикРемонта.ДатаПланирования, ПланГрафикРемонта.ПериодичностьДетализации, ПланГрафикРемонта.КоличествоПериодов);
	Иначе
	...
	ПериодНач = ДатаНачалаПланирования;
	ПериодКон = ДатаОкончанияПланирования;
...
Процедура ПериодНачПриИзменении(Элемент)
	...
	Если ПериодНач < ДатаНачалаПланирования Или ПериодНач > ДатаОкончанияПланирования Тогда
...
Процедура ПериодКонПриИзменении(Элемент)
	...
	Если ПериодКон < ДатаНачалаПланирования или ПериодКон > ДатаОкончанияПланирования Тогда
		ТекстСообщения = НСтр("ru = 'Введенная дата находится за пределами возможного диапазона.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		ПериодКон = ДатаОкончанияПланирования;
		Возврат;
...
Функция ПолучитьОграниченияПоОтборам(ППР)
	ОграниченияОтборов = Новый Структура;
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	торо_ПланГрафикРемонтаОбъектыРемонта.ОбъектРемонтныхРабот КАК ОР
	               |ИЗ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	торо_ПланГрафикРемонтаОбъектыРемонта.ВидРемонтныхРабот КАК ВР
	               |ИЗ
	               |	Документ.торо_ПланГрафикРемонта.ОбъектыРемонта КАК торо_ПланГрафикРемонтаОбъектыРемонта
	               |ГДЕ
	               |	торо_ПланГрафикРемонтаОбъектыРемонта.Ссылка = &Ссылка";
	Запрос.УстановитьПараметр("Ссылка", ППР);
