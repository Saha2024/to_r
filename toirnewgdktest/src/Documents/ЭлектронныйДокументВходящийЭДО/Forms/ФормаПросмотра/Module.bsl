...
Процедура ПодготовитьОтражениеВУчете()
	...
	Если СоставПакета.Количество() > 0 Тогда
		НаКонтроле = НоменклатураКонтрагентовНаКонтролеПоЭлектронномуДокументу(СоставПакета.ВыгрузитьЗначения());
	Иначе
		НаКонтроле = НоменклатураКонтрагентовНаКонтролеПоЭлектронномуДокументу(Объект.Ссылка);
	КонецЕсли;
...
Процедура ЗавершитьСопоставлениеНоменклатурыНаСервере()
	...
	НаКонтроле = НоменклатураКонтрагентовНаКонтролеПоЭлектронномуДокументу(ОбъектыКонтроля);
	ЕстьНоменклатураНаКонтроле = ЗначениеЗаполнено(НаКонтроле);
