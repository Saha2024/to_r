...
Процедура ИзменитьСоставДополнительныхСведений(Команда)
	...
	Иначе
		...
		// ТОИР++
		Если ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.торо_ОбъектыРемонта") Тогда
			ПараметрыПерехода.Вставить("Набор", ПолучитьПредопределенныйНабор("Справочник_торо_ОбъектыРемонта_Общие"));
			ПараметрыПерехода.Вставить("Свойство", Неопределено);
		КонецЕсли;
		// ТОИР--
...
// ТОИР++
&НаСервере
Функция ПолучитьПредопределенныйНабор(ИмяНабора)
	УправлениеСвойствами.НаборСвойствПоИмени(ИмяНабора);
КонецФункции
// ТОИР--
