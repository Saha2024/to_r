...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.ОбъектРемонта) = Тип("СправочникСсылка.торо_ОбъектыРемонта") Тогда
		ОрганизацияОР = Параметры.ОбъектРемонта.Организация;
		НаправлениеОР = Параметры.ОбъектРемонта.Направление;
	ИначеЕсли ТипЗнч(Параметры.ОбъектРемонта) = Тип("СправочникСсылка.торо_СписокОбъектовРегламентногоМероприятия") Тогда 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список, "ПодразделениеИсполнитель",	Параметры.ПодразделениеИсполнитель);
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.КоличестваНеДостаточноДляЗамены Тогда
		...
		ТекстПредупреждения = СтрШаблон(ШаблонПредупреждения, 
			...
			?(ЗначениеЗаполнено(ТекущиеДанные.УпаковкаМатериала),ТекущиеДанные.КоличествоУпаковокМатериалаПоДокументу * Кратность,
...
Процедура ПриИзмененииКоличества()
	...
	Если Количество = 0 или Количество > ИсходноеКоличествоЕд Тогда
...
Процедура ОбновитьПредставлениеОбластиДействия(ТекущиеДанные)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ВидРемонта)
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ТехКарта) Тогда
			...
			ПредставлениеОбластиДействияРем = ПредставлениеОбластиДействияРем + СтрШаблон(ШаблонПредставления, ТекущиеДанные.ТехКарта);
		КонецЕсли;
	КонецЕсли;	
