...
Функция ВыполнитьКонтрольСопоставленияНоменклатуры(Знач ДанныеЭлектронногоДокумента = Неопределено,
	...
	Для Каждого НоменклатураКонтрагента Из НаСопоставление Цикл
		...
		Для Каждого Элемент Из Сопоставление Цикл
			...
			ИсключаемыеСвойства = ИсключаемыеИзКонтроляСопоставленияСвойстваНоменклатуры(ДанныеДокумента);
			Для Каждого ЭлементСвойства Из ИсключаемыеСвойства Цикл
				ДанныеДокумента.Удалить(ЭлементСвойства);
				ДанныеБазы.Удалить(ЭлементСвойства);
			КонецЦикла;
			
			Если ОбщегоНазначения.ДанныеСовпадают(ДанныеДокумента, ДанныеБазы)
				И (НоменклатураКонтрагента.ЕдиницаИзмерения = НоменклатураКонтрагентаИБ.НаименованиеУпаковки
				Или ПустаяСтрока(НоменклатураКонтрагента.ЕдиницаИзмерения)) Тогда
				ЕстьСопоставление = Истина;
...
Процедура ЗаполнитьСостояниеНеНачатогоЭДООбъектаУчета(Параметры, ДанныеСостояния) Экспорт
	...
	Иначе
		...
		ДанныеСостояния.СуществуетАктуальныйДокумент =
			ЗначениеЗаполнено(ИнтеграцияЭДО.ОсновнойЭлектронныйДокументОбъектаУчета(Параметры[0].ОбъектУчета));
		ДанныеСостояния.ПредставлениеСостояния = НСтр("ru = 'Не начат'");
...
Процедура ПередЗаписьюОбъектаДокументаЭДО(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ОписаниеИзменений = ОписаниеИзмененийОснованияЭлектронногоДокумента(Источник, Отказ);
...
//
Функция ОписаниеИзмененийОснованияЭлектронногоДокумента(Знач ОбъектУчета, Отказ)
...
Функция ИсключаемыеИзКонтроляСопоставленияСвойстваНоменклатуры(ДанныеДокумента)
	
	Результат = Новый Массив;
	
	Результат.Добавить("СтавкаНДС");
	Результат.Добавить("Штрихкод");
	Результат.Добавить("ШтрихкодКомбинации");
	Результат.Добавить("ШтрихкодыНоменклатуры");
	Результат.Добавить("ИсторияИдентификаторов");
	Результат.Добавить("НаименованиеУпаковки");
	Результат.Добавить("ЕдиницаИзмерения");
	Результат.Добавить("ТипНоменклатурыФНС");
	Результат.Добавить("КодНоменклатуры");
	Результат.Добавить("КодПоКТРУ");
	Результат.Добавить("КодТНВЭД");
	Результат.Добавить("СтранаПроисхожденияКод");
	Результат.Добавить("ПрослеживаемыйТовар");
	Результат.Добавить("МаркируемыйТовар");
