...
// 
// Параметры:
// 	ОписаниеКонтейнера - СтрокаТаблицыЗначений из см. СервисЭДО.НоваяТаблицаДокументов
...
Функция РаспаковатьФайлКонтейнера(Файл, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для каждого РаспакованныйФайл Из РаспакованныеФайлы Цикл
		...
		Если НРег(РаспакованныйФайл.Имя) = "meta.xml" Тогда
			ОписаниеКонтейнера.Метаданные = ПрочитатьФайлМетаданных(РаспакованныйФайл.ПолноеИмя,
				КаталогРаспаковки);
		КонецЕсли;
...
Функция ПараметрыКонтейнера(ИдентификаторДокументооборота, ИдентификаторСообщения, ИдентификаторПакета,
	СпособОбмена, ИдентификаторУчетнойЗаписи) Экспорт
...
// * АдресРесурсаКонтрагента - Неопределено, Строка
...
Функция СформироватьДанныеОбъектов(ТранспортныеКонтейнеры, КонтекстДиагностики = Неопределено)
	...
	РеквизитыКонтейнеров = "Статус, Организация, Контрагент, Отправитель, Получатель, ДоговорКонтрагента, СпособОбменаЭД, ВнешнийУИД, ИдентификаторПакета, ИдентификаторДокументооборота";
	...
	Попытка
		Для Каждого ДанныеОбъекта Из ДанныеОбъектов Цикл
			...
			Если Не ЗначениеЗаполнено(ОписаниеКонтейнера.Ссылка) Тогда
				...
				|АдресРесурсаОрганизации, АдресРесурсаКонтрагента, ИдентификаторДокументооборота");
