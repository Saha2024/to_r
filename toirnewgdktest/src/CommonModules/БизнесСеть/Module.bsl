...
//
// Параметры:
//  НатуральныеИдентификаторы	 - Массив - массив из натуральных ключей (см. НатуральныйИдентификаторОрганизации).
//  Отказ						 - Булево - признак отказа.
// 
// Возвращаемое значение:
...
Функция ДанныеОрганизацийПоНатуральнымИдентификаторам(НатуральныеИдентификаторы, Отказ = Ложь) Экспорт
	...
	// Делим на порции по 50 шт
	Порции = Новый Массив;
	Если НатуральныеИдентификаторы.Количество() <= 50 Тогда
		Порции.Добавить(НатуральныеИдентификаторы);
	Иначе
		Порции.Добавить(Новый Массив);
		Для ИндексЭлемента = 0 По НатуральныеИдентификаторы.ВГраница() Цикл
			Порции[Порции.ВГраница()].Добавить(НатуральныеИдентификаторы[ИндексЭлемента]);
			
			Если (ИндексЭлемента + 1) % 50 = 0 Тогда
				Порции.Добавить(Новый Массив);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого Порция Из Порции Цикл
		ПараметрыМетода = Новый Структура;
		ПараметрыМетода.Вставить("НатуральныеИдентификаторы", Порция);
		...
		ДесериализованныеДанныеСервиса = ДанныеОрганизацийИзJSON(ДанныеСервиса);
		Если ТипЗнч(ДесериализованныеДанныеСервиса) = Тип("Массив") Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, ДесериализованныеДанныеСервиса);
		КонецЕсли;
	КонецЦикла;
...
Функция РеквизитыУчастника(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Если Отказ Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка чтения данных в 1С:Бизнес-сеть:'") + " " + ТекстОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Функция ПараметрыКомандыОтправитьДокумент(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	ПараметрыОтправитель = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(ПараметрыМетода.Отправитель);
	
	Если ТипЗнч(ПараметрыМетода.Получатель) = Тип("Структура") Тогда
		ПараметрыПолучатель = ПараметрыМетода.Получатель;
	Иначе
		ПараметрыПолучатель = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(ПараметрыМетода.Получатель);
	КонецЕсли;
	...
	Если Отказ Тогда
		...
		ТекстЖурналаРегистрации = НСтр("ru = 'Ошибка отправки документов через 1С:Бизнес-сеть:'") + " " + ТекстОшибки;
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстЖурналаРегистрации, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
		Возврат Неопределено;
	КонецЕсли;
...
//
Процедура УстановитьСтатусЗагружен(Организация, Идентификатор, Ссылка, Отказ = Ложь) Экспорт
...
Функция ВидыДокументовСервиса() Экспорт
	...
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот,          НСтр("ru = 'Акт выполненных работ'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.АктНаПередачуПрав,       НСтр("ru = 'Акт передачи прав'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ОтветНаЗаказ,            НСтр("ru = 'Заказ покупателя'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ЗаказТовара,             НСтр("ru = 'Заказ поставщику'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.СоглашениеОбИзмененииСтоимости,
		НСтр("ru = 'Корректировка стоимости'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная,          НСтр("ru = 'Товарная накладная'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ОтчетОПродажахКомиссионногоТовара,
		НСтр("ru = 'Отчет о продажах комиссионного товара'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ОтчетОСписанииКомиссионногоТовара,
		НСтр("ru = 'Отчет о списании комиссионного товара'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ПрайсЛист,               НСтр("ru = 'Прайс-лист'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.СчетНаОплату,            НСтр("ru = 'Счет на оплату'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.КоммерческоеПредложение, НСтр("ru = 'Коммерческое предложение'"));
	СписокВидовЭД.Добавить(Перечисления.ТипыДокументовЭДО.ЗапросКоммерческихПредложений,
...
Процедура ЗаполнитьРеквизитыКонтрагентов(Данные) Экспорт
	...
	РеквизитНаименование = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("НаименованиеКонтрагента");
...
// Возвращает признак подключения  организации к Бизнес-сети
// и проверяет необходимость повторного подключения организации к Бизнес-сети.
// 
// Параметры:
...
Функция ОрганизацияНеПодключенаТребуетсяПовторноеПодключение(Организация) Экспорт
	Возврат Не ОрганизацияПодключена(Организация) Или ТребуетсяПовторноеПодключениеОрганизации(Организация);
КонецФункции
...
// ОбменСКонтрагентами.ПодключаемыеКомандыЭДО
...
Процедура ПриОпределенииСпискаКомандЭДО(Знач СоставКоманд, Знач ПолноеИмя, Знач НаправлениеЭД, КомандыЭДО) Экспорт
	...
	ИспользоватьОбменБизнесСеть = ИспользоватьОбменБизнесСеть();
	ЕстьПравоОбменаБизнесСеть = ПравоВыполненияОбменаДокументами();
	...
	ЕстьПравоОбменаЭД = ОбменСКонтрагентами.ЕстьПравоВыполненияОбмена();
		...
		Если ЕстьПравоОбменаЭД Тогда
			...
			Если ИспользоватьОбменБизнесСеть И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
	...
	Если СоставКоманд.Исходящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ПравоДоступа("Вывод", Метаданные) И ИспользоватьОбменБизнесСеть
			 И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
			КомандаЭДО = КомандыЭДО.Добавить();
...
Процедура ОбработатьРезультатПолученныхДокументовЧерезБизнесСеть(Знач Результат, Знач РежимИсходящихДокументов, ТаблицаДокументов = Неопределено) Экспорт
	...
	Если ТаблицаДокументов = Неопределено Тогда
		...
		ТаблицаДокументов.Колонки.Добавить("НаправлениеЭД",  Новый ОписаниеТипов("ПеречислениеСсылка.НаправленияЭДО"));
		...
		ТаблицаДокументов.Колонки.Добавить("ВидДокумента", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыДокументовЭДО"));
	КонецЕсли;
	...
	Для Каждого ЗначениеМассива Из Результат Цикл
		...
		Если ТипЗнч(ЗначениеМассива) <> Тип("Структура") ИЛИ Не ЗначениеМассива.Свойство("documentTitle") Тогда
			...
			ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, ТекстОшибки, "БизнесСеть");
			Возврат;
		КонецЕсли;
		...
		НоваяСтрока.НаправлениеЭД = ?(РежимИсходящихДокументов, ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Исходящий"),
...
Функция СсылкаДокументаИБПоДаннымСервиса(Знач ВидЭДСервиса, Знач ИдентификаторВнутренний, Знач РежимИсходящихДокументов,
	...
	ТипДокумента = ОбменСКонтрагентамиИнтеграция.ТипДокументаПоЗначениюСервиса(ВидЭДСервиса);
	Если ТипДокумента <> Неопределено Тогда
		...
		Если Не РежимИсходящихДокументов Тогда
			...
			СписокТипов = ИнтеграцияЭДО.ТипыОбъектовУчетаПоТипуЭлектронногоДокумента(ВидДокумента);
...
Функция ТаблицаЭлектронныхДокументовСформированныхЧерезБизнесСеть(Знач СсылкиНаДокументы, Знач ОрганизацияЗарегистрирована,
	УникальныйИдентификатор, Отказ, ТекстОшибки = Неопределено) Экспорт
	...
	ТаблицаЭД.Колонки.Добавить("Организация");
	ТаблицаЭД.Колонки.Добавить("ПубличнаяСсылкаQRКода");
	ТаблицаЭД.Колонки.Добавить("ИзображениеQRКода");
	ТаблицаЭД.Колонки.Добавить("ИдентификаторВременнойСсылки");
	ТаблицаЭД.Колонки.Добавить("АдресХранилищаКартинкиQRКода");
	ТаблицаЭД.Колонки.Добавить("ОрганизацияЗарегистрирована");
	
	ОписанияОбъектовУчета = ОбменСКонтрагентамиИнтеграция.ОписанияОбъектовУчета(СсылкиНаДокументы);
	
	ДанныеQRКодовОбъектовУчета = Новый Соответствие;
	ПараметрыВизуализацииОбъектовУчета = Новый Соответствие;
	Если ВыводитьQRКодНаПечатныхФормахДокументов() Тогда
		Для Каждого ОписаниеОбъектаУчета Из ОписанияОбъектовУчета Цикл
			ЕстьОшибки = Ложь;
			ДанныеQRКода = ДанныеQRКодаПоДокументу(ОписаниеОбъектаУчета.ОбъектУчета, ОписаниеОбъектаУчета.Организация, ЕстьОшибки);
			Если Не ЕстьОшибки Тогда
				ДанныеQRКодовОбъектовУчета.Вставить(ОписаниеОбъектаУчета.ОбъектУчета, ДанныеQRКода);
				ПараметрыВизуализации = ОбменСКонтрагентамиИнтеграция.НовыеПараметрыВизуализации();
				ПараметрыВизуализации.ПубличнаяСсылкаQRКода = ДанныеQRКода.ПубличнаяСсылкаQRкода;
	...
	Если НЕ ЗначениеЗаполнено(ДанныеДляВыгрузки) Или Отказ Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ВидыДокументовСервиса = ВидыДокументовСервиса();
	
	Для Каждого СтрокаТаблицы Из ДанныеДляВыгрузки Цикл
		
		Если ВидыДокументовСервиса.НайтиПоЗначению(СтрокаТаблицы.ТипДокумента) = Неопределено Тогда
		...
		НоваяСтрока.ПолноеИмяФайла = СтрокаТаблицы.ПолноеИмяФайла;
		НоваяСтрока.НаименованиеФайла = СтрокаТаблицы.Наименование;
		НоваяСтрока.НаправлениеЭД = СтрокаТаблицы.Направление;
		НоваяСтрока.Контрагент = СтрокаТаблицы.Контрагент;
		НоваяСтрока.УникальныйИдентификатор = СтрокаТаблицы.ОбъектУчета.УникальныйИдентификатор();
		НоваяСтрока.ВладелецЭД = СтрокаТаблицы.ОбъектУчета;
		НоваяСтрока.ПолноеИмяДопФайла = СтрокаТаблицы.ПолноеИмяДопФайла;
		
		НоваяСтрока.ВидЭД = СтрокаТаблицы.ТипДокумента;
		НоваяСтрока.Сумма = СтрокаТаблицы.Сумма;
		НоваяСтрока.Организация = СтрокаТаблицы.Организация;
		НоваяСтрока.ИдентификаторВременнойСсылки = УникальныйИдентификатор;
		НоваяСтрока.ОрганизацияЗарегистрирована = ОрганизацияЗарегистрирована;
		
		НоваяСтрока.АдресХранилища = ПоместитьВоВременноеХранилище(
			СтрокаТаблицы.ДвоичныеДанныеПакета,
			УникальныйИдентификатор);
		НоваяСтрока.АдресХранилищаПредставления = ПоместитьВоВременноеХранилище(
			ПредставлениеДокументаДляОтправки(СтрокаТаблицы.ДвоичныеДанныеПредставления),
			УникальныйИдентификатор);
		
		ДанныеQRКода = ДанныеQRКодовОбъектовУчета[СтрокаТаблицы.ОбъектУчета];
		Если ДанныеQRКода <> Неопределено Тогда
			НоваяСтрока.ИзображениеQRКода = ДанныеQRКода.ИзображениеQRКода;
			НоваяСтрока.ПубличнаяСсылкаQRКода = ДанныеQRКода.ПубличнаяСсылкаQRКода;
			НоваяСтрока.АдресХранилищаКартинкиQRКода = ПоместитьВоВременноеХранилище(ДанныеQRКода.ИзображениеQRКода,
				УникальныйИдентификатор);
		КонецЕсли;
...
Функция ОтправитьДанныеДокументовЧерезБизнесСетьВФоне(Знач УникальныйИдентификатор, Знач ИдентификаторЗадания, Знач ДополнительныеПараметры) Экспорт
...
//                                      содержит НовыйДанныеОтветаОтправкиДокументаИПолученияQRКода().
//
Процедура ОтправитьДанныеДокументовЧерезБизнесСеть(Знач ДополнительныеПараметры, АдресХранилищаРезультата) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокументов Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ДвоичныеДанныеПакета) Тогда
			СтрокаТаблицы.ДвоичныеДанныеПакета = ПолучитьИзВременногоХранилища(СтрокаТаблицы.АдресХранилища);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ДвоичныеДанныеПредставления) Тогда
			СтрокаТаблицы.ДвоичныеДанныеПредставления = 
				ПолучитьИзВременногоХранилища(СтрокаТаблицы.АдресХранилищаПредставления);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИдентификаторыСервисаПоДокументам) Тогда
		ДанныеОтвета = НовыйДанныеОтветаОтправкиДокументаИПолученияQRКода();
		ДанныеОтвета.ДанныеQRКодовПоДокументам         = ДанныеQRКодовПоДокументам;
		ДанныеОтвета.ИдентификаторыСервисаПоДокументам = ИдентификаторыСервисаПоДокументам;
	Иначе
...
Процедура ОтправитьДокументИПолучитьQRКодЧерезБизнесСеть(Знач СсылкиНаДокументы, АдресХранилищаРезультата) Экспорт
	...
	ТаблицаЭД = ТаблицаЭлектронныхДокументовСформированныхЧерезБизнесСеть(
		СсылкиНаДокументы, Истина, Новый УникальныйИдентификатор, Отказ, ТекстОшибки);
	
	Если Отказ Или ТаблицаЭД = Неопределено Тогда
		ДанныеОтвета = НовыйДанныеОтветаОтправкиДокументаИПолученияQRКода();
		ДанныеОтвета.Ошибки.Добавить(ТекстОшибки);
		ПоместитьВоВременноеХранилище(ДанныеОтвета, АдресХранилищаРезультата);
		Возврат;
	КонецЕсли;
...
Функция ОбновитьКлючДоступа(ИдентификаторОрганизации, ТокенОбновления, Отказ = Ложь)
	...
	Если РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Обновление ключа доступа. Работа с внешними ресурсами заблокирована.'"), 
			Подсистема, 
			УровеньЖурналаРегистрации.Информация);
	Иначе
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Обновление ключа доступа. Работа с внешними ресурсами не заблокирована.'"), 
			Подсистема, 
			УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	...
	ШаблонЗаписи = НСтр("ru = 'Обновление ключа доступа. Запрос: %1 %2'");
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
		СтрШаблон(ШаблонЗаписи, ПараметрыКоманды.Метод, ПараметрыКоманды.Адрес), 
		Подсистема, 
		УровеньЖурналаРегистрации.Информация);
	...
	ШаблонЗаписи = НСтр("ru = 'Обновление ключа доступа. Ответ: 
		| Срок действия токена доступа %1; 
		| Срок действия токена обновления %2'");
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
		СтрШаблон(ШаблонЗаписи, Результат.СрокДействияТокенаДоступа, Результат.СрокДействияТокенаОбновления), 
		Подсистема, 
		УровеньЖурналаРегистрации.Информация);
	...
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
		НСтр("ru = 'Обновление ключа доступа. Ключ записан.'"), 
		Подсистема, 
		УровеньЖурналаРегистрации.Информация);
...
Функция ПараметрыКомандыОбновитьКлючДоступа(ПараметрыМетода)
	...
	ПараметрыКоманды.Ошибки.Вставить(
		403, 
		ТекстОшибкиПотериДоступа(ПараметрыКоманды.Наименование));
...
Функция ПараметрыКомандыПолучитьКодАктивацииВБизнесСети(ПараметрыМетода)
	...
	ПричинаОшибки = "Недостаточно прав для выполнения операции. Токен обновления Бизнес-сети инвалидирован.";
...
Функция ПараметрыКомандыУдалитьДокументы(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Иначе
		РеквизитыОрганизации = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(ПараметрыМетода.Организация);
		ПараметрыКоманды.ИдентификаторОрганизации = ИдентификаторОрганизации(ПараметрыМетода.Организация);
	КонецЕсли;
	...
	Если Отказ Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка удаления документов в 1С:Бизнес-сеть:'") + " " + ТекстОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыКомандыОтправкаПриглашенийКонтрагентам(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Для каждого РеквизитыКонтрагента Из ПараметрыМетода.Данные Цикл
		...
		Если Отказ Тогда
			...
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка отправки приглашения в 1С:Бизнес-сеть:'") + " " + ТекстОшибки, 
				ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
	...
	Если Отказ Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстОшибки, ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Функция ПараметрыКомандыОтправкаПриглашения(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	ПараметрыОрганизации = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(ПараметрыМетода.Организация);
	ПараметрыКонтрагента = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(ПараметрыМетода.Контрагент);
...
Функция ПараметрыКомандыОтправитьУведомлениеОбОтправке(ПараметрыМетода)
	...
	РеквизитыКонтрагента = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(ПараметрыМетода.Получатель); 
...
Функция ПараметрыКомандыСписокВходящихДокументов(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Для каждого ЗначениеМассива Из СписокОрганизаций Цикл
		...
		Если Отказ Тогда
			...
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка чтения данных 1С:Бизнес-сеть по организации:'") + " "
				+ ТекстОшибки, 
				ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
	...
	Если ПараметрыМетода.Свойство("Контрагент") Тогда
		...
		Если Отказ Тогда
			...
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка отбора по контрагенту при чтении документов 1С:Бизнес-сеть:'") + " " + ТекстОшибки, 
				ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Функция ОбработатьОтветСервиса(ПараметрыСоединения, ПараметрыКоманды, КодСостояния, Данные, Отказ)
	...
	Если ОписаниеОшибки = Неопределено
		...
		Если КодСостояния >= 500 Тогда
			ОписаниеОшибки = НСтр("ru = 'Внутренняя ошибка сервиса'");
		ИначеЕсли КодСостояния >= 400 Тогда
			ОписаниеОшибки = НСтр("ru = 'Ошибка параметров запроса'");
		КонецЕсли;
		ОписаниеОшибки = ПараметрыКоманды.Ошибки.Получить(КодСостояния);
	КонецЕсли;
	...
	Если ОписаниеОшибки <> Неопределено Тогда
		...
		Если ТипЗнч(ОписаниеОшибки) = Тип("Булево") И Не ОписаниеОшибки Тогда
			Если ЗначениеЗаполнено(Данные) Тогда
				ОписаниеОшибки = НСтр("ru = 'Ошибка сервиса'");
			Иначе
				Возврат Неопределено;
			КонецЕсли;
		ИначеЕсли ТипЗнч(ОписаниеОшибки) = Тип("Булево") И ОписаниеОшибки Тогда
			Возврат Неопределено;
		КонецЕсли;
		...
		Если ВыводитьПользователюСообщениеОбОшибках(ПараметрыКоманды) Тогда
			ЭлектронноеВзаимодействие.ОбработатьОшибку(ПараметрыКоманды.Наименование,
				ПодробныйТекстОшибки, ОписаниеОшибки, "БизнесСеть");
		Иначе
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ПодробныйТекстОшибки, ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
		КонецЕсли;
...
Процедура ЗагрузитьДокументы(Знач ПараметрыЗапроса, Знач АдресРезультата) Экспорт
	...
	Для каждого СтрокаЗагрузки Из ПараметрыЗапроса Цикл
		...
		ПараметрыОтраженияВУчете.Отправитель = СтрокаЗагрузки.Контрагент;
		ПараметрыОтраженияВУчете.ТипДокумента = СтрокаЗагрузки.ВидЭД;
		ПараметрыОтраженияВУчете.Направление = СтрокаЗагрузки.НаправлениеЭД;
		ПараметрыОтраженияВУчете.ДанныеОсновногоФайла = ПолучитьИзВременногоХранилища(СтрокаЗагрузки.ДанныеФайлаРазбора);
		ПараметрыОтраженияВУчете.ДанныеДополнительногоФайла = ПолучитьИзВременногоХранилища(СтрокаЗагрузки.ДанныеФайлаДопДанных);;
		ПараметрыОтраженияВУчете.СпособОбработки = СтрокаЗагрузки.ТипОбъекта;
		ПараметрыОтраженияВУчете.ДополнительныеДанные.Вставить("ПрайсЛист", ПрайсЛистПоИдентификаторам(СтрокаЗагрузки.ИдентификаторыПрайсЛистов));
		
		МассивОбъектов = Новый Массив;
		Если ЗначениеЗаполнено(СтрокаЗагрузки.СсылкаНаДокумент) Тогда
			МассивОбъектов.Добавить(СтрокаЗагрузки.СсылкаНаДокумент);
		КонецЕсли;
		
		ОбменСКонтрагентамиИнтеграция.ОтразитьВУчетеЭлектронныйДокумент(ПараметрыОтраженияВУчете, МассивОбъектов, Отказ);
		
		Если Не ЗначениеЗаполнено(СтрокаЗагрузки.СсылкаНаДокумент) И ЗначениеЗаполнено(МассивОбъектов) Тогда
			СтрокаЗагрузки.СсылкаНаДокумент = МассивОбъектов[0];
		КонецЕсли;
	...
	Иначе
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
	КонецЕсли;
...
Функция ПрайсЛистПоИдентификаторам(ИдентификаторыПрайсЛистов)
...
Функция КлючДоступаБизнесСети(
	...
	Исключение
		...
		СообщитьОбОшибкеЗаписатьВЖурнал("НеУдалосьПрочитатьОбновитьКлючДоступа", Отказ, ИнформацияОбОшибке);
	КонецПопытки;
...
Функция ДанныеОрганизации(Организация)
	...
	Результат = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(Организация);
...
Процедура СообщитьОбОшибкеЗаписатьВЖурнал(ТипСообщения, Отказ = Ложь, ИнформацияОбОшибке = "")
	
	Если ТипСообщения = "ТокенОбновленияИнвалидирован" Тогда
		
		ПричинаОшибки = НСтр("ru = 'Токен обновления Бизнес-сети инвалидирован.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			ПричинаОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
			
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибкиПотериДоступа(ПричинаОшибки, Истина));
		
	ИначеЕсли ТипСообщения = "ЗакончилсяСрокТокенаОбновления" Тогда
		
		ПричинаОшибки = НСтр("ru = 'Закончен период действия токена обновления Бизнес-сети.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			ПричинаОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
	...
	ИначеЕсли ТипСообщения = "НеУдалосьПрочитатьОбновитьКлючДоступа" Тогда
		...
		ПричинаОшибки = НСтр("ru = 'Не удалось прочитать или обновить ключ доступа Бизнес-сети.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			ПричинаОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Процедура ЗаполнитьРеквизитыОрганизаций(Данные)
	...
	РеквизитНаименование = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("НаименованиеОрганизации");
...
Функция ПредставлениеДокументаДляОтправки(ДвоичныеДанныеПечатнойФормы)
...
// Возвращает структуру данных ответа отправки документа и получения QR-кода.
//
// Возвращаемое значение:
...
//      ** Значение - Структура из см. БизнесСеть.НовыйДанныеQRКода.
...
//   * Ошибки                            - Массив из Строка - описание ошибок.
//
Функция НовыйДанныеОтветаОтправкиДокументаИПолученияQRКода()
	
	ДанныеОтвета = Новый Структура;
	ДанныеОтвета.Вставить("ДанныеQRКодовПоДокументам");
	ДанныеОтвета.Вставить("ИдентификаторыСервисаПоДокументам");
	ДанныеОтвета.Вставить("Ошибки", Новый Массив());
