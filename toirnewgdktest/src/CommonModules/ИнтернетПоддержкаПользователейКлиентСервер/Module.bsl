...
Функция ПроверитьДанныеАутентификации(ДанныеАутентификации) Экспорт
	...
	НеДопустимыеСимволы = ЛогинСодержитНеДопустимыеСимволы(ДанныеАутентификации.Логин);
	Если НеДопустимыеСимволы <> "" Тогда
		Результат.Отказ = Истина;
		Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В поле ""Логин"" присутствуют недопустимые символы: %1.'"),
			НеДопустимыеСимволы);
		Результат.Поле = "Логин";
	КонецЕсли;
...
//
// Возвращаемое значение:
//  Строка - недопустимые символы.
//
Функция ЛогинСодержитНеДопустимыеСимволы(Логин)
	
	ДопустимыеСимволы = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz01234567890@_./\-";
	
	НеДопустимыеСимволы = Новый Массив;
	Для Итератор = 1 По СтрДлина(Логин) Цикл
		Символ = Сред(Логин, Итератор, 1);
		Если СтрНайти(ДопустимыеСимволы, Символ) = 0 Тогда
			НеДопустимыеСимволы.Добавить(Символ);
		КонецЕсли;
	КонецЦикла;
	
	НеДопустимыеСимволы = ОбщегоНазначенияКлиентСервер.СвернутьМассив(
		НеДопустимыеСимволы);
	
	Возврат СтрСоединить(НеДопустимыеСимволы, " ");
