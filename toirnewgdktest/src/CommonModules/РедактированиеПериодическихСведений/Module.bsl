////////////////////////////////////////////////////////////////////////////////
// РедактированиеПериодическихСведений: методы, позволяющие редактировать периодические 
//	сведения базы данных.  
////////////////////////////////////////////////////////////////////////////////

#Область ПрограммныйИнтерфейс

// Читает набор записей на форму.
// Параметры:
//		Форма - ФормаКлиентскогоПриложения - форма.
//		ИмяРегистра - Строка - Имя регистра.
//		ВедущийОбъект - Произвольный - значение ключевого измерения регистра для отбора.
...
//		Форма - ФормаКлиентскогоПриложения - форма.
//		ИмяРегистра - Строка - Имя регистра.
//		СтруктураВедущихОбъектов - Структура - значения ключевых измерений регистра для отбора.
...
//		Форма - ФормаКлиентскогоПриложения - управляемая форма,
//		ИмяРегистра - Строка - имя редактируемого регистра,
//		СтруктураВедущихОбъектов - Структура - структура ссылок на ведущие объекты.
//
...
// Получает менеджер последней записи регистра сведений.
//	Параметры:
//		ИмяРегистра - Строка - имя регистра.
//		СтруктураВедущихОбъектов - Структура - значения ключевых измерений регистра для отбора.
...
//		Форма - ФормаКлиентскогоПриложения - управляемая форма
//		ИмяРегистра - Строка - имя редактируемого регистра.
//		ВедущийОбъект - Произвольный - значение ключевого измерения регистра.
//
...
//		Форма - ФормаКлиентскогоПриложения - форма.
//		ИмяРегистра - Строка - имя редактируемого регистра.
//		СтруктураВедущихОбъектов - Структура - Структура ссылок на ведущий объект.
//
...
//		ИмяРегистра - Строка - имя редактируемого регистра.
//		ВедущийОбъект - Произвольный - значение ключевого измерения регистра.
//		ИменаКолонокИсключаемыхИзСравнения - Строка - имена исключаемых колонок.
//		ДополнительныеСвойства - Структура  - структура доп. свойств.
//
// Возвращаемое значение:
//		Булево - данные были изменены.
...
Процедура ПроверитьЗаполнениеПоляЗаписиРегистраВФорме(МенеджерЗаписи, ИмяРегистра, ОписаниеПоля, Синоним, Отказ, ПутьКРеквизитуФормы = "")
	...
	Если ОписаниеПоля.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку И Не ЗначениеЗаполнено(МенеджерЗаписи[ОписаниеПоля.Имя]) Тогда
		ШаблонСообщения = НСтр("ru = '%1: не заполнено поле ""%2"".'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Синоним, ?(ЗначениеЗаполнено(ОписаниеПоля.Синоним), ОписаниеПоля.Синоним, ОписаниеПоля.Имя));
...
Функция ЗаписатьНаборЗаписейПослеРедактированияВФормеПоСтруктуре(Форма, ИмяРегистра, СтруктураВедущихОбъектов, ИменаКолонокИсключаемыхИзСравнения, ДополнительныеСвойства)
	...
	Если НЕ ОбщегоНазначения.КоллекцииИдентичны(ТаблицаИсходногоНабора, ТаблицаНовогоНабора, ,ИменаКолонокИсключаемыхИзСравнения) Тогда
		...
		Для Каждого СтрокаТаблицаНовогоНабора Из ТаблицаНовогоНабора Цикл
			...
			Иначе
				Если ОбщегоНазначения.КоллекцииИдентичны(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаТаблицаНовогоНабора), ОбщегоНазначения.СтруктураПоМенеджеруЗаписи(МенеджерЗаписи, Метаданные.РегистрыСведений[ИмяРегистра]), , ИменаКолонокИсключаемыхИзСравнения) Тогда
