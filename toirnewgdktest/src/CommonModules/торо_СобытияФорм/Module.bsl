...
// Процедура, вызываемая из одноименного обработчика события формы. Для уставновки свойств 
// элементов формы в зависимости от доступных прав рекомендуется использовать процедуру "УстановитьСвойствоЭлементовФормыОтПрав"
// текущего модуля
//
// Параметры:
...
Функция ДокументыОснованияДокументОснованиеНачалоВыбора(Форма) Экспорт
	...
	Если Элемент <> Неопределено Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'Возможность ввода данного документа на основании отключена в настройках ввода на основании
...
// Процедура управляет свойствами элементов формы в зависимости от доступных прав.
//
// Параметры:
//  Форма					- ФормаКлиентскогоПриложения - форма для которой необходимо настроить свойства элементов формы.
//
Процедура УстановитьСвойствоЭлементовФормыОтПрав(Форма) Экспорт
...
// Подготавливает структуру с именами таблиц формы и их реквизитами для дальнейшего
// сохранения в нее данных строк до изменения.
//
// Параметры:
//		Форма - ФормаКлиентскогоПриложения - форма для которой необходимо сохранять данные строк до изменения.
//		ИменаТаблицФормы - Массив - имена таблиц формы для которых необходимо сохранять данные строк до изменения.
// Возвращаемое значение:
//		Структура - структура с имена таблиц формы и их реквизитами.
Функция ПодготовитьСтруктуруСохраненныхСтрок(Форма, ИменаТаблицФормы) Экспорт
	
	СтруктураДанных = Новый Структура;
	
	Для Каждого ИмяТаблицыФормы Из ИменаТаблицФормы Цикл
		
		РеквизитыТаблицы = Новый Массив;
		
		ПутьКДанным = Форма.Элементы[ИмяТаблицыФормы].ПутьКДанным;
		
		РеквизитыФормы = Форма.ПолучитьРеквизиты(ПутьКДанным); 
		Для Каждого Реквизит Из РеквизитыФормы Цикл
			РеквизитыТаблицы.Добавить(Реквизит.Имя);
		КонецЦикла;	
		
		СтруктураДанных.Вставить(ИмяТаблицыФормы);
		СтруктураДанных.Вставить(ИмяТаблицыФормы + "Реквизиты", РеквизитыТаблицы);
...
Функция ПолучитьТабСоответствияФормИПроцедурЗаполненияОписанияКнопок()
	...
	НС = ТабСоответствия.Добавить();
	НС.ИмяФормы = Метаданные.Документы.торо_УстановкаКоэффициентовРемонтныхОсобенностей.Формы.ФормаДокумента.ПолноеИмя();
	НС.ИмяПроцедуры = "ЗаполнитьОписаниеЭлементовФормыДокументаУстановкаКоэффициентовРемонтныхОсобенностей";
...
Процедура ЗаполнитьОписаниеЭлементовФормыЭлементаОР(ТабОписанияКнопок)
	...
	ИменаКнопокДляПраваРедактирование.Добавить("ИзменитьПоложениеВИерархии");
	ИменаКнопокДляПраваРедактирование.Добавить("ТаблицаОценокРМЗаполнитьВидыРемонтовИзНормативов");
	ИменаКнопокДляПраваРедактирование.Добавить("ТаблицаОценокРМЗаполнитьТаблицуРМ");
	ИменаКнопокДляПраваРедактирование.Добавить("ТиповойСоставПроставитьУстановленныеОР");
	ИменаКнопокДляПраваРедактирование.Добавить("ДеревоОграниченийДобавитьИерархию");
	ИменаКнопокДляПраваРедактирование.Добавить("ДеревоОграниченийДобавитьОграничение");
	...
	ИменаКнопокДляПраваРедактирование.Добавить("ДеревоОграничений");
	ИменаКнопокДляПраваРедактирование.Добавить("ТиповойСостав");
...
Процедура ЗаполнитьОписаниеЭлементовФормыДокументаВыявленныхДефектов(ТабОписанияКнопок)
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "СписокДефектовСоздатьТиповойДефект", "Редактирование", Метаданные.Справочники.торо_ТиповыеДефектыОборудования.ПолноеИмя(), "Доступность", Ложь);
...
Процедура ЗаполнитьОписаниеЭлементовФормыЭлементаТиповогоОР(ТабОписанияКнопок)
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ДеревоОграничений", "Редактирование", ТекОбъектМетаданныхПолноеИмя, "ТолькоПросмотр", Истина);
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ТаблицаОценокРМЗаполнитьТаблицуРМ", "Редактирование", ТекОбъектМетаданныхПолноеИмя, "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ДеревоОграниченийДобавитьИерархию", "Редактирование", ТекОбъектМетаданныхПолноеИмя, "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ДеревоОграниченийДобавитьОграничение", "Редактирование", ТекОбъектМетаданныхПолноеИмя, "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ФормаСкопировать", "ИнтерактивноеДобавление", ТекОбъектМетаданныхПолноеИмя, "Доступность", Ложь);
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "СписокОРДобавитьОРВСостав", "Редактирование", Метаданные.Справочники.торо_ОбъектыРемонта.ПолноеИмя(), "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "СписокОРСоставОРУдалить", "Редактирование", Метаданные.Справочники.торо_ОбъектыРемонта.ПолноеИмя(), "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "СоставОРОчиститьОР", "Редактирование", Метаданные.Справочники.торо_ОбъектыРемонта.ПолноеИмя(), "Доступность", Ложь);
...
Процедура ЗаполнитьОписаниеЭлементовФормыЭлементаТехОперации(ТабОписанияКнопок)
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "Перерассчитать", "Редактирование", ТекОбъектМетаданныхПолноеИмя, "Доступность", Ложь);
...
Процедура ЗаполнитьОписаниеЭлементовФормыДокументаВнешнееОснование(ТабОписанияКнопок)
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ДеревоРемонтныхРабот", "Редактирование", ТекОбъектМетаданныхПолноеИмя, "ТолькоПросмотр", Истина);
...
Процедура ЗаполнитьОписаниеЭлементовФормыДокументаППР(ТабОписанияКнопок)
	...
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ПланРемонтовКонтекстноеМенюЗаявкаНаРемонт", "ИнтерактивноеДобавление", Метаданные.Документы.торо_ЗаявкаНаРемонт.ПолноеИмя(), "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ПланРемонтовКонтекстноеМенюНарядНаВыполнениеРемонтныхРабот", "ИнтерактивноеДобавление", Метаданные.Документы.торо_НарядНаВыполнениеРемонтныхРабот.ПолноеИмя(), "Доступность", Ложь);
	ДобавитьСтрокуСОписаниемЭлементаФормы(ТабОписанияКнопок, "ПланРемонтовКонтекстноеМенюАктОВыполненииРемонтныхРабот", "ИнтерактивноеДобавление", Метаданные.Документы.торо_АктОВыполненииЭтапаРабот.ПолноеИмя(), "Доступность", Ложь);
...
Процедура ЗаполнитьОписаниеЭлементовФормыДокументаУстановкаКоэффициентовРемонтныхОсобенностей(ТабОписанияКнопок)

	ТекОбъектМетаданныхПолноеИмя = Метаданные.Документы.торо_УстановкаКоэффициентовРемонтныхОсобенностей.ПолноеИмя();
