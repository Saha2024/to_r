...
// * КонтрактЕИС - ПеречислениеСсылка.ТипыДокументовЭДО - Контракт ЕИС.
// * ДокументПриемкиЕИСУПД - ПеречислениеСсылка.ТипыДокументовЭДО - документ о приемке ЕИС УПД.
// * ДокументПриемкиЕИСУКД - ПеречислениеСсылка.ТипыДокументовЭДО - документ о приемке ЕИС УКД.
// * РезультатРассмотренияАктаПриемкиЕИСУПД - ПеречислениеСсылка.ТипыДокументовЭДО - результат рассмотрения акта ЕИС.
// * РезультатРассмотренияАктаПриемкиЕИСУКД - ПеречислениеСсылка.ТипыДокументовЭДО - результат рассмотрения акта ЕИС. 
// * СведенияОРеализацииКомиссионером - Перечисления.ТипыДокументовЭДО.СведенияОРеализацииКомиссионером.
// * СведенияОЗакупкеКомиссионером - Перечисления.ТипыДокументовЭДО.СведенияОЗакупкеКомиссионером.
// * КорректировкаСведенийОРеализацииКомиссионером - Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОРеализацииКомиссионером.
// * КорректировкаСведенийОЗакупкеКомиссионером - Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОЗакупкеКомиссионером.
// * ВозвратТоваровМеждуОрганизациями - Перечисления.ТипыДокументовЭДО.ВозвратТоваровМеждуОрганизациями.
// * ПередачаТоваровМеждуОрганизациями - Перечисления.ТипыДокументовЭДО.ПередачаТоваровМеждуОрганизациями.
//
Функция СтандартныеТипыДокументов() Экспорт
	...
	ТипыДокументов.Вставить("РезультатРассмотренияАктаПриемкиЕИСУПД", Перечисления.ТипыДокументовЭДО.РезультатРассмотренияАктаПриемкиЕИСУПД);
	ТипыДокументов.Вставить("РезультатРассмотренияАктаПриемкиЕИСУКД", Перечисления.ТипыДокументовЭДО.РезультатРассмотренияАктаПриемкиЕИСУКД);
...
Функция СостояниеПакета(СостоянияДокументов) Экспорт
	...
	Если ТипЗнч(СостоянияДокументов) = Тип("Массив") Тогда
		МассивСостояний = СостоянияДокументов;
	Иначе
...
Функция ПредставлениеДанныхСообщенияПоСсылке(Сообщение, СообщениеОтвета = Неопределено, ПараметрыВизуализации = Неопределено, КонтекстДиагностики = Неопределено) Экспорт
	...
	ИначеЕсли ОписаниеВида.ТипДокумента = Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность
		...
		Возврат МашиночитаемыеДоверенности.ТабличныйДокументМЧД(ДвоичныеДанныеФайла);
...
//                          - Неопределено
//   * РезультатПроверкиПоМЧД - см. НовыйРезультатПроверкиПодписиПоМЧД
//                            - Неопределено
...
Функция ЗаписатьРезультатПроверкиПодписейДокумента(ПроверенныеПодписи) Экспорт
	...
	Для Каждого РезультатПроверкиФайла Из ПроверенныеПодписи Цикл
		...
		Для Каждого СвойстваПодписи Из УстановленныеПодписи Цикл
			...
			СвойстваПодписи.Комментарий = РезультатПроверки.Комментарий;
			...
			Если СвойстваПодписи.Сертификат = Неопределено Тогда
				СвойстваПодписи.Сертификат = РезультатПроверки.Сертификат;
				СвойстваПодписи.Отпечаток = РезультатПроверки.Отпечаток;
				СвойстваПодписи.КомуВыданСертификат = РезультатПроверки.КомуВыданСертификат;
			КонецЕсли;
			
			РезультатПроверкиПоМЧД = ПроверитьПодписьФайлаПоМЧД(РезультатПроверкиФайла.Ключ, СвойстваПодписи);
			
			ЗаписатьРезультатПроверкиПодписиФайла(РезультатПроверкиФайла.Ключ, СвойстваПодписи, РезультатПроверкиПоМЧД);
...
Функция УстановленныеПодписиСУчетомДоверенностей(Сообщение) Экспорт
	...
	Для Каждого СвойстваПодписи Из УсиленныеКвалифицированныеПодписи Цикл
		...
		ХешиПодписей.Добавить(ХешПодписи);
		ДанныеПодписейПоХешам.Вставить(ХешПодписи, ДанныеПодписи);
...
Функция ДвоичныеДанныеСообщения(Сообщение)
	...
	Запрос.Текст =
		...
		|	И ПрисоединенныеФайлы.Расширение В (""xml"", ""zip"")
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";
...
// 
// Возвращаемое значение:
//  Структура - Проверить подписи файлов:
// * Успех - Булево
// * ПодписиДляПроверки - См.ПодписиДокументаДляПроверкиНаКлиенте
Функция ПроверитьПодписиФайлов(ПрисоединенныеФайлы, КонтекстДиагностики)
	...
	Для Каждого ДанныеФайла Из ДвоичныеДанныеФайлов Цикл
		...
		Для Каждого СвойстваПодписи Из УстановленныеПодписи Цикл
			...
			РезультатПроверкиПоМЧД = ПроверитьПодписьФайлаПоМЧД(ДанныеФайла.Ключ, СвойстваПодписи);
			
			ЗаписатьРезультатПроверкиПодписиФайла(ДанныеФайла.Ключ, СвойстваПодписи, РезультатПроверкиПоМЧД);
...
Функция ТребуетсяМашиночитаемаяДоверенностьПоДаннымДокумента(ИННДоверителя, ДанныеСертификата)
	...
	Результат = МашиночитаемыеДоверенностиКлиентСервер.ТребуетсяМашиночитаемаяДоверенность(
...
Функция ПроверитьПодписьФайлаПоМЧД(ФайлСообщения, СвойстваПодписи)
	...
	Запрос.Текст =
		...
		|	ПрисоединенныеФайлы.ПолноеИмяФайла КАК ПолноеИмяФайла,
		|	СообщениеЭДО.Направление КАК Направление,
		|	ЭлектронныйДокументВходящийЭДО.Контрагент КАК Доверитель
		|ИЗ
		...
		|		ПО СообщениеЭДО.ЭлектронныйДокумент = ЭлектронныйДокументВходящийЭДО.Ссылка
		|ГДЕ
		|	ПрисоединенныеФайлы.Ссылка = &ФайлСообщения
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ПрисоединенныеФайлы.ПолноеИмяФайла КАК ПолноеИмяФайла,
		|	СообщениеЭДО.Направление КАК Направление,
		|	ЭлектронныйДокументИсходящийЭДО.Организация КАК Доверитель
		|ИЗ
		|	Справочник.СообщениеЭДОПрисоединенныеФайлы КАК ПрисоединенныеФайлы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СообщениеЭДО КАК СообщениеЭДО
		|		ПО ПрисоединенныеФайлы.ВладелецФайла = СообщениеЭДО.Ссылка
	...
	РезультатПроверки = ПроверитьПодписьПоМЧД(ФайлСообщения, Выборка.Направление, ОписаниеФайла, СвойстваПодписи,
		Выборка.Доверитель, ПроверкиОператоромЭДО);
...
Функция ПроверитьПодписьПоМЧД(ФайлСообщения, НаправлениеСообщения, ОписаниеФайла, СвойстваПодписи,
	...
	Если Не ЗначениеЗаполнено(НомераМЧД) Тогда
		...
		НомераМЧД =
			МашиночитаемыеДоверенности.НомераДоверенностейПоУчастникам(СведенияОДоверителе, СведенияОПредставителе);
...
Функция ПараметрыПроверкиПодписиПоМЧД(НомераМЧД, СвойстваПодписи, ИННДоверителя, НаборСведенийОСтатусахМЧД, ЭтоМЧДКонтрагента)
	...
	Если СведенияОСтатусеМЧД = Неопределено Тогда
		ПараметрыПроверки.ТекстОшибки =
			НСтр("ru = 'Произошла неизвестная ошибка при получении статус МЧД из распределенного реестра'");
		Возврат ПараметрыПроверки;
...
Функция ОписаниеПакетаПоДаннымОбъектов(ДанныеОбъектовПакета)
	...
	ЕстьРаспакованные = Ложь;
	ИдентификаторыДокументооборотов = Новый Массив;
	Для каждого ДанныеОбъекта Из ДанныеОбъектовПакета.Значение Цикл
		Если ДанныеОбъекта.Распакован Тогда
			ЕстьРаспакованные = Истина;
		КонецЕсли;
		ИдентификаторыДокументооборотов.Добавить(ДанныеОбъекта.ИдентификаторДокументооборота);
	КонецЦикла;
	
	Если Не ЕстьРаспакованные Тогда
	...
	Запрос.Текст = 
		...
		|	Документ.ЭлектронныйДокументВходящийЭДО КАК ЭлектронныйДокументВходящийЭДО
		|ГДЕ
		|	ЭлектронныйДокументВходящийЭДО.ИдентификаторДокументооборота В(&ИдентификаторыДокументооборотов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	ПакетыДокументовЭДО.Организация КАК Организация,
		|	ПакетыДокументовЭДО.Контрагент КАК Контрагент,
		|	ПакетыДокументовЭДО.ДоговорКонтрагента КАК ДоговорКонтрагента
		|ИЗ
		|	РегистрСведений.СоставПакетовДокументовЭДО КАК СоставПакетовДокументовЭДО
		...
		|ГДЕ
		|	СоставПакетовДокументовЭДО.ЭлектронныйДокумент В
		|			(ВЫБРАТЬ
		|				ЭлектронныеДокументы.Ссылка КАК Ссылка
		|			ИЗ
		|				ЭлектронныеДокументы КАК ЭлектронныеДокументы)";
	
	Запрос.УстановитьПараметр("ИдентификаторыДокументооборотов", ИдентификаторыДокументооборотов);
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ОписаниеПакета, ВыборкаДетальныеЗаписи);
	КонецЕсли;
...
Функция СформироватьОтвет(ЭлектронныйДокумент, КонтекстДиагностики)
	...
	Попытка
		...
		РезультатПроверки = ФорматыЭДО.ПроверитьДокумент(ОписаниеФайла);
		...
		СообщениеОбъект = СоздатьОтветноеСообщение(ПараметрыДокумента.ВидДокумента, СвойстваОсновногоФайла,
			ЭлектронныйДокумент, ПараметрыДокумента, НаборОбъектовУчета, 
...
Функция СоздатьОтветноеСообщение(ВидСообщения, СвойстваОсновногоФайла, ЭлектронныйДокумент, ПараметрыДокумента, НаборОбъектовУчета, 
	ОшибкиФормирования, ДанныеРучногоФормированияОтветногоТитула = Неопределено)
	...
	Если (МодульОбменСГИСЭПД <> Неопределено 
		И МодульОбменСГИСЭПД.ЭтоТипРегламентаЭПД(ЭлектронныйДокумент.ТипРегламента)) Тогда	
		ОписаниеСообщения = МодульОбменСГИСЭПД.ОписаниеСообщенияОтправителя(ЭлектронныйДокумент);
	ИначеЕсли Не ЗначениеЗаполнено(ОписаниеДанных.Данные.Строки) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗапросДанныхСообщенийДляПодписания(ПараметрыВыполнения, КонтекстДиагностики)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СертификатыУчетныхЗаписей.ИдентификаторЭДО, """") КАК ИдентификаторОрганизации,
		|	ЕСТЬNULL(СертификатыУчетныхЗаписей.Доверенность, ЗНАЧЕНИЕ(Справочник.МашиночитаемыеДоверенностиОрганизаций.ПустаяСсылка)) КАК Доверенность
		|ИЗ
		|	ДоступныеСертификаты КАК ДоступныеСертификаты
...
Функция ОбработатьРезультатыЗапросаДанныхМЧДДляПодписания(ПараметрыВыполнения, РезультатыЗапроса, КонтекстДиагностики)
	...
	Пока Выборка.Следующий() Цикл
		...
		НаборСертификатов = СоздатьНаборСертификатовДляПодписанияМЧД(
			Выборка.Организация, Выборка.ИдентификаторОрганизации, СертификатыДляПодписания, ОтпечаткиНаСервере, Ошибки);
...
Функция ОбработатьРезультатыЗапросаДанныхСообщенийДляПодписания(ПараметрыВыполнения, РезультатыЗапроса, КонтекстДиагностики)
	...
	НомерСтроки = СертификатыДляПодписания.Количество() - 1;  
	
	Пока НомерСтроки >= 0 Цикл
		
		СтрокаТЗ = СертификатыДляПодписания[НомерСтроки];
		
		Если ЗначениеЗаполнено(СтрокаТЗ.Доверенность)
			И НЕ МашиночитаемыеДоверенности.ДоверенностьДействительна(СтрокаТЗ.Доверенность, ТекущаяДатаСеанса()) Тогда
			СертификатыДляПодписания.Удалить(СтрокаТЗ);
		КонецЕсли;
...
Функция СоздатьНаборСертификатовДляПодписанияМЧД(Организация,
	...
	Отбор = Новый Структура;
	Отбор.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
	...
	Если ДанныеСертификатов.Количество() > 0 Тогда
		...
		УстановитьПривилегированныйРежим(Истина);
		ПаролиСертификатов = 
			КриптографияБЭД.ПаролиСертификатов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Сертификат));
		УстановитьПривилегированныйРежим(Ложь);
		
		НовыйНаборСертификатов.ПаролиСертификатов = ПаролиСертификатов;
...
//  НаборыСертификатов - Соответствие
//  СертификатыДляПодписания - ТаблицаЗначений
	...
	Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
		...
		Если Выборка.Направление = Перечисления.НаправленияЭДО.Интеркампани
			...
			ЭтоСертификатФизическогоЛица = 
				МашиночитаемыеДоверенностиКлиентСервер.ЭтоСертификатФизическогоЛица(СвойстваСубъекта, СвойстваИздателя);
...
Функция ОписаниеНабораДанныхДляПодписанияНаСервере(ПараметрыСообщенийПоСертификатам, ДвоичныеДанныеФайлов,
	...
	Для Каждого ПараметрыСообщения Из ПараметрыСообщенийПоСертификатам.Значение Цикл
		...
		Если СодержаниеДокумента <> Неопределено Тогда
			...
			ТребуетсяМЧД = МашиночитаемыеДоверенностиКлиентСервер.ТребуетсяМашиночитаемаяДоверенность(
				СодержаниеДокумента.Отправитель.ИНН, СвойстваСубъектаСертификата, СвойстваИздателяСертификата);
			...
			Если ТребуетсяМЧД Тогда
				...
				ВыбраннаяМЧД = МашиночитаемыеДоверенности.ПолучитьПоследнююВыданнуюДоверенностьОрганизации(ОтборМЧД);
				Если ЗначениеЗаполнено(ВыбраннаяМЧД) И ФорматыЭДО.ЗаполнениеДанныхДоверенностиДоступно(СодержаниеДокумента) Тогда
					ОписаниеФайла = ФорматыЭДО.ЗаполнитьДанныеДоверенностейДокумента(ОписаниеФайла, ПараметрыСообщения.Организация,
						ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыбраннаяМЧД), СодержаниеДокумента);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		ДанныеСообщения.ВыбраннаяМЧД = ВыбраннаяМЧД;
		
		Если ДанныеСообщения.ТребуетсяМЧД И Не ЗначениеЗаполнено(ДанныеСообщения.ВыбраннаяМЧД)
...
Функция ОписаниеНабораДанныхДляИнтерактивногоПодписания(ПараметрыСообщенийПоСертификатам, ДвоичныеДанныеФайлов)
	...
	Для Каждого ПараметрыСообщения Из ПараметрыСообщенийПоСертификатам.Значение Цикл
		...
		Если Не (ПараметрыСообщения.ПодписанЭП
			ИЛИ НРег(ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(
		...
		Для Каждого ДанныеСертификата Из ПараметрыСообщенийПоСертификатам.Ключ.ДанныеСертификатов Цикл
			...
			ТребуетсяДоверенность = МашиночитаемыеДоверенностиКлиентСервер.ТребуетсяМашиночитаемаяДоверенность(
				ПараметрыСообщения.ОрганизацияИНН, СвойстваСубъектаСертификата, СвойстваИздателяСертификата);
...
//                                - ДвоичныеДанные
//
Функция ЗаполнитьПодписантовИДоверенностиПоСертификату(ДанныеДляЗаполнения, Сертификат, КонтекстДиагностики) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПараметрыЗаполнения.ТребуетсяЗаполнитьПодписанта
			...
			ОписаниеФайла = ФорматыЭДО.ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла, Выборка.Организация,
				Сертификат, СодержаниеДокумента);
...
Процедура ДополнитьДанныеОбъектовДляЗагрузки(ДанныеОбъектов)
	...
	ДанныеОбъектов.Колонки.Добавить("НомераМЧД");
...
Процедура ЗагрузитьДанныеОбъектовПакета(ДанныеОбъектовПакета, ПроверенныеПодписи, КонтекстДиагностики, РезультатЗагрузки)
	...
	Попытка
		...
		ОписаниеПакета = ОписаниеПакетаПоДаннымОбъектов(ДанныеОбъектовПакета);
...
Функция ЗагрузитьСообщениеДокумента(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики)
	...
	ДобавитьМашиночитаемыеДоверенности(СообщениеОбъект.ОсновнойФайл, ДанныеОбъекта.Содержание, ДанныеОбъекта.НомераМЧД);
...
Процедура ДобавитьМашиночитаемыеДоверенности(ОсновнойФайлСообщения, СодержаниеФайла, НомераМЧД)
