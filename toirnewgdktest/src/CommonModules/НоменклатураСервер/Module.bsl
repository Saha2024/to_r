...
//  		*СуффиксХарактеристики - Строка - суффикс реквизита "Характеристика",
//						например, если в ТЧ используется "ХарактеристикаОприходование"
...
Процедура ПроверитьЗаполнениеХарактеристик(Объект,МассивНепроверяемыхРеквизитов,Отказ,ПараметрыПроверки = Неопределено) Экспорт
	...
	|	ТаблицаТоваров.Характеристика"+СуффиксХарактеристики+" КАК Характеристика
	|ПОМЕСТИТЬ СтрокиСОшибками
	|ИЗ
	|	&ТаблицаТоваров КАК ТаблицаТоваров
	|ГДЕ
	|	ТаблицаТоваров.Характеристика"+СуффиксХарактеристики+" = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)";
	...
	Запрос.УстановитьПараметр("ТаблицаТоваров",  Объект[ИмяТЧ].Выгрузить(СписокСтрок, "НомерСтроки,Номенклатура,Характеристика"+СуффиксХарактеристики));
	...
	ПредставлениеРеквизитаХарактеристика = МетаданныеОбъекта.ТабличныеЧасти[ИмяТЧ].Реквизиты["Характеристика"+СуффиксХарактеристики].Синоним; //МетаданныеОбъекта.ТабличныеЧасти[ИмяТЧ].Реквизиты.Характеристика.Синоним;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НеЗаполненаХарактеристика Тогда
			...
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ИмяТЧ, Выборка.НомерСтроки, "Характеристика"+СуффиксХарактеристики);
...
Процедура ПроверитьВидНоменклатурыОприходования(Объект, Отказ, ИмяТЧДляПроверки = "Товары") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = НСтр("ru='Вид номенклатуры другого качества (испорченного товара) должен совпадать с видом исходной номенклатуры, т.к. ведется учет по серям номенклатуры'");
