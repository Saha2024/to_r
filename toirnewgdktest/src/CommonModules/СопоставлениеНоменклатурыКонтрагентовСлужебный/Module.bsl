...
Функция ПолучитьСловаДляЗаписиВСловарьСопоставленияНоменклатурыБЭД(Знач СоответствиеЗначенийНаименований,
	Знач ТипОбъекта, НаборСсылок = Неопределено) Экспорт
...
//  ТребуетсяПоискНоменклатуры        - Булево - признак необходимости выполнять поиск номенклатуры.
// 
// Возвращаемое значение:
...
Функция ПодобратьВариантыСопоставленияНоменклатуры(Знач ТаблицаНоменклатурыКонтрагентов,
	...
	Если ТребуетсяПоискНоменклатуры Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
			...
			МодульПодсистемы.ПодобратьВариантыСопоставленияПоНатуральнымКлючам(ТаблицаНоменклатурыКонтрагентов, ПроцентТочностиПоискаНаименований);
		КонецЕсли;
