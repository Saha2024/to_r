...
Процедура ПечатьДокументовВыполнитьКоманду(Форма, Команда, ПродолжитьВыполнениеНаСервере, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Команда.Имя = "ЭДОСформироватьПроизвольныйДокумент" Тогда
		...
		Если НастройкиПечатныхФорм.Количество() = 1 Тогда 
			...
			ПараметрыФормы.Вставить("Уникальность", НастройкиПечатныхФорм[0].Название);	
		...
		Иначе
			...
			Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
				...
				ПараметрыДокумента.Вставить("Уникальность", НастройкаПечатнойФормы.Название);
			...
			ТекстВопроса = СтрШаблон(ШаблонВопроса, ДанныеДокументов.Количество());
			
			Оповещение = Новый ОписаниеОповещения(
				"ПослеОтветаНаВопросОФормированииДокументов", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ПослеОтветаНаВопросОФормированииДокументов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Для Каждого Документ Из ДополнительныеПараметры.ДанныеДокументов Цикл
			...
			КонецЕсли;
			Если Документ.Свойство("Уникальность") Тогда
				ПараметрыФормы.Вставить("Уникальность", Документ.Уникальность);
			КонецЕсли;
...
Процедура ПослеВыбораПечатнойФормы(Результат, ОбъектПечати) Экспорт
	...
	Если РезультатПечати.Количество() = 1 Тогда
		...
		ПараметрыФормы.Вставить("Расширение", "pdf");
		ПараметрыФормы.Вставить("Уникальность", РезультатПечати[0].ИмяФайла);
	...
	Иначе
		...
		Для Каждого ПечатныйДокумент Из РезультатПечати Цикл
			...
			ПараметрыДокумента.Вставить("Расширение", "pdf");
			ПараметрыДокумента.Вставить("Уникальность", РезультатПечати[0].ИмяФайла);
...
// Открывает новый произвольный электронный документ по сформированным данным.
// 
// Параметры:
//  Основание - ЛюбаяСсылка - основание привязывается для типов в составе см. ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  АдресОписанияФайла - Строка - ссылка на временное хранилище значения см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
Процедура ОткрытьНовыйПроизвольныйЭлектронныйДокументПоСформированнымДанным(Основание, АдресОписанияФайла) Экспорт
...
// Обработчик общих для форм документа, справоничка, списка событий
// 
// Параметры:
//  ИмяСобытия - Строка
//  Параметр - Произвольный
...
Процедура СформироватьИДобавитьПроизвольныйДокументВПакетПолучитьРеквизиты(ДополнительныеПараметры, ПараметрыФайла)
	...
	ОткрытьВводРеквизитовПроизвольногоДокумента(Оповещение, ПараметрыОповещения);
