...
Процедура ПриОпределенииНаличияПрисоединенныхФайлов(ВладелецФайлов, ЕстьФайлы, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ВладелецФайлов) = Тип("СправочникСсылка.торо_ИдентификаторыТехКарт") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ТехКарта = РегистрыСведений.торо_ВерсииТехКарт.ПолучитьВерсиюТехКарты(ВладелецФайлов);
		
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		|	Файлы.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.торо_ТехКартыПрисоединенныеФайлы КАК Файлы
		|ГДЕ
		|	Файлы.ВладелецФайла = &ВладелецФайлов
		|	И НЕ Файлы.ПометкаУдаления";
		
		Запрос.УстановитьПараметр("ВладелецФайлов", ТехКарта);
...
Процедура ПриПолученииПрисоединенныхФайлов(ВладелецФайлов, ДанныеФайлов, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ВладелецФайлов) = Тип("СправочникСсылка.торо_ИдентификаторыТехКарт") Тогда
		...
		МодульРаботаСФайлами.ЗаполнитьПрисоединенныеФайлыКОбъекту(ТехКарта, Файлы);
...
Процедура ЗаполнитьРеквизитыИзПотребителя(Прокси, ОбъектXDTO, СсылкаНаПотребитель) Экспорт
	...
	ТипXDTOПользователь = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОбъектXDTO, "DMUser");
	ТипXDTOСотрудник = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОбъектXDTO, "DMEmployee"); 
	
	Если ТипЗнч(СсылкаНаПотребитель) = Тип("СправочникСсылка.Пользователи") 
		И (ТипXDTOПользователь ИЛИ ТипXDTOСотрудник)  Тогда
		
		Если ТипXDTOПользователь 
			И Не ОбъектXDTO.subdivision = Неопределено Тогда
			Возврат;
		ИначеЕсли ТипXDTOСотрудник 
			И (Не ОбъектXDTO.subdivision = Неопределено ИЛИ Не ОбъектXDTO.position = Неопределено) Тогда
			Возврат;
		КонецЕсли;	
		...
		|			ФизическоеЛицо В
		|				(ВЫБРАТЬ
		|					ДанныеПользователя.ФизическоеЛицо КАК ФизическоеЛицо
		|				ИЗ
		|					ДанныеПользователя КАК ДанныеПользователя)) КАК КадроваяИсторияСотрудниковСрезПоследних";
		...
		Исключение
			// В исключение уйдем только в случае, если недостаточно прав для чтения данных.
			// В этом случае будет ошибка создания пользователя в ДО 3.0.
			ТекстСообщения = НСтр("ru 'Недостаточно прав для получения подразделения пользователя.'");
		...
		Если Выборка.Следующий() Тогда 
			...
			Если ОбъектXDTO.subdivision = Неопределено Тогда 
			...
			Если ТипXDTOСотрудник И ОбъектXDTO.position = Неопределено Тогда 
				ОбъектXDTO.position = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
					Прокси,
					"DMPosition",
					Выборка.Должность);
			КонецЕсли;	
...
Процедура ПриИзмененииСостоянияСогласования(ПредметСогласования, Состояние, ВызовИзФормыОбъекта) Экспорт
	...
	Если ТипЗнч(ПредметСогласования) = Тип("СправочникСсылка.торо_ТехКарты")
		И Состояние = Перечисления.СостоянияСогласованияВДокументообороте.Согласован Тогда
		...
		|ГДЕ
		|	торо_ВерсииТехКарт.ТехКарта = &ТехКарта";
		
		Запрос.УстановитьПараметр("ТехКарта", ПредметСогласования);
		
		РезультатЗапроса = Запрос.Выполнить();
		Выборка = РезультатЗапроса.Выбрать();
		
		Если Выборка.Следующий() Тогда
			Если Выборка.ИдентификаторТехКарты <> Перечисления.торо_СтатусыНормативныхРемонтовИТехКарт.Действует Тогда
				ИдентификаторТехКартыОбъект = Выборка.ИдентификаторТехКарты.ПолучитьОбъект();
				ИдентификаторТехКартыОбъект.Статус = Перечисления.торо_СтатусыНормативныхРемонтовИТехКарт.Действует; 
				
				Попытка 
					ИдентификаторТехКартыОбъект.Записать();
				Исключение 
					ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Ошибка записи идентификатора тех. карты после согласования.'"),
					УровеньЖурналаРегистрации.Ошибка,
					ИдентификаторТехКартыОбъект,
					ИдентификаторТехКартыОбъект.Ссылка,
...
Процедура ПриОпределенииЗначенияРеквизитаПоОбъектуXDTO(Результат, ЗначениеЗаполнения, ТипРеквизита, ИмяРеквизита,
	...
	// Получим представления статусов документов.
	
	Если Не ТипЗнч(ЗначениеЗаполнения) = Тип("Структура") 
		ИЛИ Не ЗначениеЗаполнения.ТипXDTOОбъекта = "DMDocument" Тогда
		Возврат;
	КонецЕсли;	
	
	ИменаСвойствСостоянийДокументов = Новый Массив;
	ИменаСвойствСостоянийДокументов.Добавить("statusApproval");
	ИменаСвойствСостоянийДокументов.Добавить("statusConfirmation");
	ИменаСвойствСостоянийДокументов.Добавить("statusSigning");
	ИменаСвойствСостоянийДокументов.Добавить("statusRegistration");
	ИменаСвойствСостоянийДокументов.Добавить("statusPerformance");
	ИменаСвойствСостоянийДокументов.Добавить("statusAcquaintance");
	
	Если ИменаСвойствСостоянийДокументов.Найти(ЗначениеЗаполнения.ИмяСвойства) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПредставленияЗначенийСостоянийДокументов = Новый Соответствие;
	ПредставленияЗначенийСостоянийДокументов.Вставить("Registered", "Зарегистрирован");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Performed", "Исполнен");
	ПредставленияЗначенийСостоянийДокументов.Вставить("UnderDevelopment", "На доработке");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AwaitingPerformance", "На исполнении");
	ПредставленияЗначенийСостоянийДокументов.Вставить("OnFamiliarization", "На ознакомлении");
	ПредставленияЗначенийСостоянийДокументов.Вставить("InPreparation", "На подготовке");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AwaitsSigning", "На подписании");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AwaitingReviewal", "На рассмотрении");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AwaitingRegistration", "На регистрации");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AwaitingApproval", "На согласовании");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AwaitingConfirmation", "На утверждении");
	ПредставленияЗначенийСостоянийДокументов.Вставить("NotRegistered", "Не зарегистрирован");
	ПредставленияЗначенийСостоянийДокументов.Вставить("NotApproved", "Не согласован");
	ПредставленияЗначенийСостоянийДокументов.Вставить("NotConfirmed", "Не утвержден");
	ПредставленияЗначенийСостоянийДокументов.Вставить("AcquaintanceCompleted", "Ознакомлен");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Rejected", "Отклонен");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Signed", "Подписан");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Project", "Проект");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Considered", "Рассмотрен");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Approved", "Согласован");
	ПредставленияЗначенийСостоянийДокументов.Вставить("Confirmed", "Утвержден");
	
	ПредставлениеЗначения =  ПредставленияЗначенийСостоянийДокументов.Получить(ЗначениеЗаполнения.ЗначениеСвойства);
	Если Не ПредставлениеЗначения = Неопределено Тогда
...
Процедура ПриОпределенииСокращенногоНаименованияКонфигурации(СокращенноеНаименование) Экспорт
	...
	СокращенноеНаименование = НСтр("ru = '1С:ТОИР КОРП'");
