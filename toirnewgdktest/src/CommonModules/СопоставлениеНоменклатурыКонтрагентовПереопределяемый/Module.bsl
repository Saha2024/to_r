...
Процедура ПриИзмененииНоменклатурыСопоставления_НоменклатураКонтрагентов(Форма, ДанныеСопоставления) Экспорт
	...
	Если ДанныеСопоставления.Номенклатура.Пустая() Тогда
		...
		ДанныеСопоставления.Упаковка                    = Справочники.УпаковкиНоменклатуры.ПустаяСсылка();
	...
	РеквизитыНоменклатуры.Вставить("ИспользоватьХарактеристики"               , "ВидНоменклатуры.ИспользоватьХарактеристики");
	...
	Если ЗначениеЗаполнено(ДанныеСопоставления.Характеристика) Тогда
		...
		КорректныйВладелецХарактеристики = ?(ДанныеНоменклатуры.ИндивидуальныеХарактеристикиНоменклатуры, 
			ДанныеСопоставления.Номенклатура, ДанныеНоменклатуры.ВидНоменклатуры);
...
Процедура НайтиНаименованиеЕдиницыИзмеренияПоКодуОКЕИ(КодОКЕИЕдиницыИзмерения, НаименованиеЕдиницыИзмерения) Экспорт
	...
	ДанныеЕдиницы = Справочники.ЕдиницыИзмерения.ЕдиницаИзмеренияПоКоду(КодОКЕИЕдиницыИзмерения);
...
Процедура СформироватьТаблицуСопоставленияНоменклатурыКонтрагентовВДокументах(
	...
	|	&ПредставлениеНоменклатурыКлиента КАК ПредставлениеНоменклатурыКлиента,
	|	ЗаказПоставщикуТовары.Номенклатура КАК НоменклатураПоиск,
	|	ЗаказПоставщикуТовары.Характеристика КАК ХарактеристикаПоиск,
	|	ЗаказПоставщикуТовары.Номенклатура.ЕдиницаИзмерения КАК УпаковкаПоиск
	|ИЗ
...
//   											 См. ОбменСКонтрагентами.СпособыОтраженияВУчетеТипаЭлектронногоДокумента.
...
Процедура ЗначенияНаименованийДанныхСопоставленияНоменклатурыБЭД(Знач НаборСсылокНаОбъекты, СоответствиеЗначенийНаименований) Экспорт
	...
	СоответствиеЗначенийНаименований = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(НаборСсылокНаОбъекты, "Наименование");
...
Процедура ТекстЗапросаПоискаВариантовСопоставленияУпаковкиНоменклатурыБЭД(ТекстЗапроса) Экспорт
	...
	|			КОНЕЦ = ЕСТЬNULL(УпаковкиНоменклатуры.Коэффициент, 1)
	|			ТОГДА 100
	...
	|					ИЛИ ТаблицаНоменклатуры.НаименованиеУпаковки = Упаковки.НаименованиеПолное)
	|			ТОГДА 1000
	|		ИНАЧЕ 0
	|	КОНЕЦ + ВЫБОР
	...
	|		ПО ТаблицаНоменклатуры.ЕдиницаИзмерения = Упаковки.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиНоменклатуры КАК УпаковкиНоменклатуры
	|		ПО ТаблицаНоменклатуры.ЕдиницаИзмерения = УпаковкиНоменклатуры.ЕдиницаИзмерения
	|ГДЕ
...
Процедура ПриЗаполненииФормыНоменклатурыПоДаннымКонтрагента(Знач НоменклатураКонтрагента, Форма) Экспорт
	...
	Если ЗначениеЗаполнено(НоменклатураКонтрагента.ЕдиницаИзмеренияКод) Тогда
		Форма.Объект.ЕдиницаИзмерения = Справочники.ЕдиницыИзмерения.НайтиПоКоду(НоменклатураКонтрагента.ЕдиницаИзмеренияКод);
	ИначеЕсли ЗначениеЗаполнено(НоменклатураКонтрагента.ЕдиницаИзмерения) Тогда
		Форма.Объект.ЕдиницаИзмерения = Справочники.ЕдиницыИзмерения.НайтиПоНаименованию(НоменклатураКонтрагента.ЕдиницаИзмерения, Истина);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НоменклатураКонтрагента.СтавкаНДС) Тогда
		...
		Для каждого КлючЗначение Из СтавкиНДСПоКлючу Цикл
			Если КлючЗначение.Ключ = НоменклатураКонтрагента.СтавкаНДС Тогда
				Форма.Объект.СтавкаНДС = УчетНДСВызовСервера.СтавкаНДСПоЗначениюПеречисления(КлючЗначение.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
