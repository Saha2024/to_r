...
Процедура ЗаполнитьПараметрыАутентификации(КлючиСинхронизации, КонтекстДиагностики) Экспорт
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Если ТипЗнч(МаркерЗашифрованный) = Тип("ДвоичныеДанные") Тогда
			Если ДанныеСертификата.ПарольПолучен И КриптооперацииДоступныНаСервере Тогда
...
// * ДобавляемыйСертификат - см. ДанныеДляРегистрационногоПакета1СЭДО.ПараметрыРегистрации.СертификатКриптографии
...
Функция ДанныеДляРегистрационногоПакета1СЭДО(ПараметрыРегистрации) Экспорт
	...
	Ошибки = Неопределено;
	ДанныеРегистрационнойИнформации = ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки);
	...
	ДанныеПакета.Вставить("ДобавляемыйСертификат", ПараметрыРегистрации.СертификатКриптографии);
...
Функция ОтправитьКонтейнер(КлючСинхронизации, ОписаниеКонтейнера, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульПодсистемы.ИзменитьАдресРесурсаНаСервереДляЭПД(ОписаниеКонтейнера.Ссылка, ПараметрыВыполненияОперации.АдресРесурса);
	КонецЕсли;
...
Функция ПолучитьДокументыЭПД(КлючСинхронизации, ДатаПоследнегоЗапроса, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД")
		...
		Возврат ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, КонтекстДиагностики, Истина);
	КонецЕсли;
...
Функция ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, КонтекстДиагностики, ЭтоЭПД = Ложь)
...
// Параметры:
// 	КлючСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации
// 	Документ - СтрокаТаблицыЗначений из см. НоваяТаблицаДокументов
// 	КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// Возвращаемое значение:
...
Функция ПолучитьДокумент(КлючСинхронизации, Документ, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульПодсистемы.ИзменитьАдресРесурсаНаСервереДляЭПД(Документ.КодРегламента, ПараметрыВыполненияОперации.АдресРесурса);
	КонецЕсли;
...
// Проверяет, подключены ли абоненты к сервисы 1С-ЭДО.
// 
// Параметры:
// 	ДанныеДляПроверки - ТаблицаЗначений:
// 	  * ИНН - Строка
// 	  * КПП - Строка
// 	КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// Возвращаемое значение:
...
//   * КПП - Строка
//   * Подключен - Булево
Функция ПроверитьПодключениеАбонентовКСервису1СЭДО(Знач ДанныеДляПроверки, КонтекстДиагностики) Экспорт
	
	ДанныеДляПроверки.Колонки.Добавить("Подключен", Новый ОписаниеТипов("Булево"));
	КоличествоСтрок = ДанныеДляПроверки.Количество();
	...
	КоличествоЗапросов = ?(КоличествоСтрок % РазмерПорции = 0, КоличествоСтрок / РазмерПорции, Цел(КоличествоСтрок / РазмерПорции) + 1);
		
	// Выполняем несколько запросов. В каждом запросе не более 10000 строк
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		
		МинимальныйНомерКонтрагента 	= Мин(РазмерПорции * (НомерПорции - 1), КоличествоСтрок);
		МаксимальныйНомерКонтрагента 	= Мин(РазмерПорции * НомерПорции, КоличествоСтрок) - 1;
		
		Поток = Новый ПотокВПамяти();
		ЗапросСостояний = Новый ЗаписьXML;
		ЗапросСостояний.ОткрытьПоток(Поток, "windows-1251");
		...
		Для ИндексТекущегоКонтрагента = МинимальныйНомерКонтрагента По МаксимальныйНомерКонтрагента Цикл
			...
			ДанныеКонтрагента = ДанныеДляПроверки[ИндексТекущегоКонтрагента];
		...
		ПараметрыВыполненияОперации.ТребуетсяМаркер = Ложь;
		ПараметрыВыполненияОперации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
		ПараметрыВыполненияОперации.ПредставлениеОперации = НСтр("ru = 'Проверка подключения абонентов к сервису 1С-ЭДО'");
		ПараметрыВыполненияОперации.АдресРесурса = "GetMassInfo";
		ПараметрыВыполненияОперации.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
		ПараметрыВыполненияОперации.ТелоЗапроса = ТелоЗапроса;
		ПараметрыВыполненияОперации.Таймаут = ИнтернетСоединениеБЭД.ТаймаутПоРазмеруФайла(ТелоЗапроса.Размер());
		...
		Если Результат.Успех Тогда
			ОтветСервиса = РаботаСФайламиБЭД.ПрочитатьXDTO(Результат.Ответ.ПолучитьТелоКакПоток());
			ОбработатьОтветСервиса(ОтветСервиса, ДанныеДляПроверки, МинимальныйНомерКонтрагента,
...
Функция ВыполнитьОперацию(ПараметрыОперации, КонтекстДиагностики,
		КлючСинхронизации = Неопределено, МаскируемыеПараметрыПриЛогировании = Неопределено)
...
//
Функция СоединениеССервисом(СпособОбмена, Таймаут = 30)
...
// * АдресРесурса - Строка -
// * МаскируемыеПараметрыПриЛогировании - Массив из Строка
Функция НовыеПараметрыВыполненияОперацииСервиса(КлючСинхронизации = Неопределено)
...
Функция СписокЭлектронныхДокументовИзОтветаОтСервиса(РезультатВыполненияОперации, ДатаПоследнегоЗапроса, ЭтоЭПД = Ложь)
...
Функция НоваяТаблицаДокументов()
	...
	ТЗ.Колонки.Добавить("ЭтоЭПД", Новый ОписаниеТипов("Булево"));
...
// * ЭлектронныеДокументы - см. НоваяТаблицаДокументов 
//
Функция НовыйРезультатПолученияСпискаДокументов()
	
	Результат = Новый Структура;
	Результат.Вставить("ЭлектронныеДокументы", НоваяТаблицаДокументов());
...
Процедура ОбработатьОтветСервиса(ОтветСервиса, ДанныеДляПроверки, МинимальныйНомер,
	МаксимальныйНомер)

	ИННКПП = ИННКППИзОтветаСервиса(ОтветСервиса);

	Если ИННКПП.Количество() > 0 Тогда

		Для ИндексТекущейСтроки = МинимальныйНомер По МаксимальныйНомер Цикл

			СтрокаТаблицы = ДанныеДляПроверки[ИндексТекущейСтроки];

			КлючПоиска = СтрокаТаблицы.ИНН + ?(ЗначениеЗаполнено(СтрокаТаблицы.КПП), "/"
				+ СтрокаТаблицы.КПП, "");

			Если ИННКПП.Найти(КлючПоиска) <> Неопределено Тогда
...
Функция ИННКППИзОтветаСервиса(ОтветСервиса)
	
	Результат = Новый Массив;
	
	Абоненты = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ОтветСервиса, "Abonent", Новый Массив);
	Для Каждого Абонент Из Абоненты Цикл
		ИНН = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Абонент, "INN", "");
		КПП = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Абонент, "KPP", "");
		ИННКППКонтрагента = ИНН + ?(ЗначениеЗаполнено(КПП), "/" + КПП,"");
		Если Результат.Найти(ИННКППКонтрагента) = Неопределено Тогда
			Результат.Добавить(ИННКППКонтрагента);
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки)
	...
	ПараметрыСертификатов = КриптографияБЭД.СвойстваСертификатов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыРегистрации.СертификатКриптографии));
	ПараметрыСертификата = ПараметрыСертификатов[ПараметрыРегистрации.СертификатКриптографии];
	...
	Если Не ИнтеграцияЭДО.ЭтоФизЛицо(ПараметрыРегистрации.Организация) Тогда
		...
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ЮЛ.НаимОрг",
			ПараметрыОрганизации[ИмяРеквизитаНаименованиеОрганизации]);
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ЮЛ.ИННЮЛ",
			СокрЛП(ПараметрыОрганизации[ИмяРеквизитаИННОрганизации]));
		...
		ЗначенияРеквизитов.Вставить("УчастЭДО.ТипУчастника.ЮЛ.ОГРН",
			СокрЛП(ПараметрыОрганизации[ИмяРеквизитаОГРНОрганизации]));
	...
	ТребуетсяДоверенность = ТипУчастника = "ЮЛ";
	...
	ЗначенияРеквизитов.Вставить("Подписант.Должность",    ДолжностьПоСертификату);
	ЗначенияРеквизитов.Вставить("Подписант.ФИО.Фамилия",  ФамилияБезПодчеркивания);
	ЗначенияРеквизитов.Вставить("Подписант.ФИО.Имя",      ИмяБезПодчеркивания);
	ЗначенияРеквизитов.Вставить("Подписант.ФИО.Отчество", ОтчествоБезПодчеркивания);
...
Функция СформироватьРегистрационнуюИнформациюCML(ДеревоДанных)
	...
	Попытка
		...
		ЗначенияСвойствXDTO.Вставить("Отчество", "Подписант.ФИО.Имя");
		ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
		ПараметрыЗаполнения.НеобязательныеСвойства = "Отчество";
		РаботаСФайламиБЭД.ЗаполнитьСвойстваXDTOИзДерева(ФИО, ЗначенияСвойствXDTO, ДеревоДанных, Ошибки,
			ПараметрыЗаполнения);
