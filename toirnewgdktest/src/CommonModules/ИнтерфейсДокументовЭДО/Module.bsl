...
Функция УстановленныеПодписи(ЭлементСхемыРегламента, ТипРегламента) Экспорт
	...
	Если ТипРегламента = Перечисления.ТипыРегламентовЭДО.Неформализованный Тогда
		Для Каждого Подпись Из Подписи Цикл
			...
			Если ЭлементСхемыРегламента.Статус = Перечисления.СтатусыСообщенийЭДО.Подтвержден
				И ЭлементСхемыРегламента.ДатаИзмененияСтатуса < СвойстваПодписи.ДатаПодписи Тогда
	...
	Если ЭлементСхемыИнформацияПолучателя <> Неопределено
		...
		ПодписиПолучателя = 
	ЭлектронныеДокументыЭДО.УстановленныеПодписиСУчетомДоверенностей(ЭлементСхемыИнформацияПолучателя.Сообщение);
...
Функция СформироватьФайлыВыгрузкиЭДВФорматеPDF(Знач ЭлектронныеДокументы, Знач ИдентификаторФормы) Экспорт
	...
	Для каждого ДанныеПечатнойФормы Из ПечатныеФормы Цикл
		...
		ИмяФайла = РаботаСФайламиБЭД.ДопустимоеИмяФайла(ДанныеПечатнойФормы.СинонимМакета, Истина);
	...
	Для Каждого СтрокаПодписи Из Подписи Цикл 
		...
		Если СтрокаПодписи.ЭтоПодписьПоДоверенности Тогда 
			Если ДоверенностьОпределена Тогда 
				...
				ПодписьВерна = СтрокаПодписи.РезультатПроверкиПоМЧД.ПодписьВерна;
...
//    * ДокументСсылка - ДокументСсылка - ссылка на документ.
//    * КонтролерСостояниеЭДО - ДекорацияФормы - элемент формы "декорация", "поле формы", в заголовке которого будет отображаться состояние ЭДО, необязательный параметр.
//    * ГруппаСостояниеЭДО - ГруппаФормы - элемент формы "группа", которому может принадлежать КонтроллерСостояниеЭДО, необязательный параметр.
//
...
Функция ЗначенияПроверокЭлектронныхПодписейПоЭлектронномуДокументу(ЭлектронныйДокумент) Экспорт
	...
	|	И ЭлектронныеПодписиПоМЧД.ПроверкаВыполнена";
...
Процедура ОбновитьЭлементыСхемыРегламента(Приемник, Источник)
	...
	Для Каждого ЭлементИсточника Из КоллекцияЭлементовИсточника Цикл
		...
		Если НЕ ДанныеЭлементаИсточника.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя И
			НЕ (МодульОбменСГИСЭПД <> Неопределено 
				И МодульОбменСГИСЭПД.ЭтоТитулОтправителя(ДанныеЭлементаИсточника.ТипЭлементаРегламента)) И
