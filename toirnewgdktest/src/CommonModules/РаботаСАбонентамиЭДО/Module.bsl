...
Процедура ПроверитьКонтрагента(Параметры) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		МодульРаботаВМоделиСервиса = Общегоназначения.ОбщийМодуль("РаботаВМоделиСервиса");
		КонтрагентДляПроверки.ОбластьДанныхВспомогательныеДанные = МодульРаботаВМоделиСервиса.ЗначениеРазделителяСеанса();
	КонецЕсли;
...
Функция РезультатПроверкиКонтрагентов(ДанныеКонтрагентов)
	...
	Запрос.Текст =
		...
		|	ВТДанныеКонтрагентов.СостояниеПоАбонентам КАК СостояниеПоАбонентам,
		|	ВТДанныеКонтрагентов.СостояниеПоПриглашениям КАК СостояниеПоПриглашениям,
		|	СостоянияКонтрагентовБЭД.Состояние КАК СостояниеТекущее,
		|	ВТДанныеКонтрагентов.ОбластьДанныхВспомогательныеДанные КАК ОбластьДанныхВспомогательныеДанные
		|ИЗ
		|	ВТДанныеКонтрагентов КАК ВТДанныеКонтрагентов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияКонтрагентовБЭД КАК СостоянияКонтрагентовБЭД
		|		ПО ВТДанныеКонтрагентов.Контрагент = СостоянияКонтрагентовБЭД.Контрагент";
	...
	ДанныеКонтрагентовДляПроверкиСервисом = РезультатПакет[1].Выгрузить();
	...
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.СостояниеТекущее)
			Или Выборка.СостояниеТекущее = СостояниеНеПодключен Тогда
			
			НоваяСтрока = ДанныеКонтрагентовДляПроверкиСервисом.Добавить();
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		Если Не ДанныеСервиса = Неопределено Тогда
			Для ИндексТекущегоКонтрагента = МинимальныйНомерКонтрагента По МаксимальныйНомерКонтрагента Цикл
				...
				ИзвестенИдентификаторАбонента = ДанныеСервиса.Найти(Отбор) <> Неопределено;
