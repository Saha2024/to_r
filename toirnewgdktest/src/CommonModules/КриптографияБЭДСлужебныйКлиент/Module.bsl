...
Процедура ПроверитьПодписьЗавершение(Результат, Контекст) Экспорт
	...
	Поток.НачатьПолучениеРазмера(Новый ОписаниеОповещения(
		"ПроверитьПодписьЗавершениеПродолжение", ЭтотОбъект, Контекст));
...
// 
// Параметры:
//  Результат - Структура - см. описание результата, вызвращаемого при выполнении оповещения, переданного в параметре
...
Процедура ПолучитьОтпечаткиСертификатов(Оповещение, Личные, ПоказатьОшибку = Ложь) Экспорт
	...
	СлужебноеОповещение = Новый ОписаниеОповещения(
		"ПослеПолученияОтпечатков", ЭтотОбъект, Контекст);
...
Функция ВыполнитьОпределениеПрограммыСертификатаКриптографии_НайтиУстановленныеПрограммы(ПроцессВыполнения)
	...
	Если ЗначениеЗаполнено(УстановленныеПрограммы) Тогда
		Если Не ЗначениеЗаполнено(НаборПрограмм) Тогда
			ПроцессВыполнения.НаборПрограмм =
				КриптографияБЭДСлужебныйВызовСервера.НайтиПрограммыЭлектроннойПодписиИШифрования(
