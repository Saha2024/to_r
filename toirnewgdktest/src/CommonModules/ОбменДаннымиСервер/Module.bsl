...
Процедура УдалитьНеактуальныеЗаписиВРегистреПравилДляОбменаДанными()
	...
	Пока Выборка.Следующий() Цикл
		...
		// +++++ТОИР
		Если Выборка.ИмяПланаОбмена = "ОбменТОИР20ТОИР30" Тогда
		    Продолжить;
		КонецЕсли;
		// -----ТОИР
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	ТекстПодзапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&ИмяТаблицыМетаданных", "ПланОбмена.ОбменТОИР20ТОИР30");
	ТекстПодзапроса = СтрЗаменить(ТекстПодзапроса, "&ИмяПланаОбмена", "ОбменТОИР20ТОИР30");
	ТекстЗапроса = ТекстЗапроса + ТекстПодзапроса;
	// ----ТОИР
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		// ТОИР++
		Если НЕ торо_СЛКВнешнееСоединение.ОбменВТекущейСерииДоступен(Выборка.УзелИнформационнойБазы) Тогда 
			Продолжить;
		КонецЕсли;
		// ТОИР--
