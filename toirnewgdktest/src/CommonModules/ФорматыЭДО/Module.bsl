...
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Форматы.БизнесСеть.Вставить("КоммерческоеПредложение", ФорматыФНС.БизнесСеть.КоммерческоеПредложение);
	Форматы.БизнесСеть.Вставить("ЗапросКоммерческогоПредложения", ФорматыФНС.БизнесСеть.ЗапросКоммерческогоПредложения);
...
Функция ФорматыЭлектронныхДокументов(Отбор = Неопределено) Экспорт
	...
	ПрикладныеФорматы = ФорматыЭДО.ПрикладныеФорматы();
...
Функция ЭтоСводныйУПД(ДвоичныеДанные) Экспорт
	...
	Если ПараметрыДокумента = Неопределено
		ИЛИ ПараметрыДокумента.ИсходныйФормат <> ФорматыЭДО.ПоддерживаемыеФорматы().ФНС.УПД2019.ИнформацияПродавца Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ЭтоМЧД(ОписаниеФайла) Экспорт
	...
	Если ОписаниеФайла.ДвоичныеДанные <> Неопределено Тогда
		...
		Возврат РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "Документ.СвДов") <> Неопределено;
	КонецЕсли;
...
//  ОписаниеФайла - см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла - описание файла для добавления подписей.
//  Организация - ОпределяемыйТип.Организация - организация.
//  СертификатПодписи - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - сертификат.
//  СодержаниеДокумента - см. НовоеОписаниеФайлаДокумента - Описание файла документа.
//
// Возвращаемое значение:
//  см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла - описание файла после добавления сведений подписанта.
//
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла, Организация, СертификатПодписи, СодержаниеДокумента) Экспорт
	...
	Если ПустаяСтрока(Фирма) 
		И ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи) Тогда
		...
		РеквизитыОрганизации.Вставить("ОКПО");
		РеквизитыОрганизации.Вставить("ДолжностьРуководителя");
		РеквизитыОрганизации.Вставить("Руководитель");
		РеквизитыОрганизации.Вставить("ЮрФизЛицо");
		РеквизитыОрганизации.Вставить("СвидетельствоДата");
		РеквизитыОрганизации.Вставить("СвидетельствоНомер");
		РеквизитыОрганизации.Вставить("ЮридическийАдрес");
		РеквизитыОрганизации.Вставить("ЗначенияПолейЮрАдрес");
		РеквизитыОрганизации.Вставить("ФактическийАдрес");
		РеквизитыОрганизации.Вставить("ЗначенияПолейФактАдрес");
		РеквизитыОрганизации.Вставить("Телефон");
		
		ОбменСКонтрагентамиПереопределяемый.ПолучитьРеквизитыОрганизацииДляВыгрузкиВФайл(Организация, 
			РеквизитыОрганизации);
		
		Фирма = РеквизитыОрганизации.НаименованиеПолное;
...
Функция СформироватьДокумент(Знач Формат, Знач Данные) Экспорт
	...
	Менеджер = МенеджерФорматов(Формат);
	Результат = Менеджер.СформироватьДокумент(Формат, Данные);
...
Функция СформироватьДанныеОсновногоТитулаПоОбъектуУчета(ОбъектУчета, Настройки, Данные) Экспорт
	...
	Если ДанныеДокумента.Строки.Количество() = 0 Тогда
...
//  См. НовыйРезультатФормированияДокументаПоУчету
//
Функция СформироватьДанныеОтветногоТитулаПоОбъектуУчета(ОбъектУчета, Формат, Данные) Экспорт
...
Функция ПредставлениеДанныхДокумента(ДанныеФайла, ДанныеФайлаОтвета = Неопределено,
	ПараметрыВизуализации = Неопределено, КонтекстДиагностики = Неопределено) Экспорт

	Возврат КонвертацияЭДО.ВизуализацияЭлектронногоДокумента(ДанныеФайла, ДанныеФайлаОтвета, ПараметрыВизуализации,
		КонтекстДиагностики);
...
Функция РазделыДополнительныхПолейФорматаЭлектронногоДокумента(ТипЭлектронногоДокумента, Формат) Экспорт
	...
	РазделыДополнительныхПолей.Колонки.Добавить("Тип");
	РазделыДополнительныхПолей.Колонки.Добавить("Имя");
	РазделыДополнительныхПолей.Колонки.Добавить("Представление");
	РазделыДополнительныхПолей.Колонки.Добавить("ПутьКЭлементуXML");
	РазделыДополнительныхПолей.Колонки.Добавить("ПутьКРеквизитуДереваДопДанные");
	РазделыДополнительныхПолей.Колонки.Добавить("ПутьКРеквизитуДереваСумма");
	
	Если Формат = Форматы.ФНС.УПД.ИнформацияПродавца
		ИЛИ Формат = Форматы.ФНС.УПД2019.ИнформацияПродавца Тогда
	...
	ИначеЕсли Формат = Форматы.ФНС.УКД.ИнформацияПродавца
	 	Или Формат = Форматы.ФНС.УКД2020.ИнформацияПродавца Тогда
...
Функция РасшифровкаВариантовЗаполненияФорматовЭДО() Экспорт
	...
	НоваяСтрока.Формат = "BNCommerceOfferingRequest";
...
Функция ДополнитьДокументДаннымиДополнительныхПолей(НастройкаЗаполнения, ДокументДляИспользованияВФормулах, ПараметрыПолучения, ДеревоДанных, КэшДополнительныхДанных)
	...
	ПараметрыЗаполнения.Вставить("ДеревоДанных",        ДеревоДанных);
...
Функция ВычислитьЗначенияДополнительныхПолейТаблицы(ПараметрыЗаполнения, ОшибкиВычисления)
	...
	НайденныеСтроки = ПараметрыЗаполнения.ДеревоДанных.Строки.НайтиСтроки(ПараметрыОтбора, Истина);
...
Функция НовоеОписаниеУчастникаЭДО()
...
//
// Параметры:
// 	ПараметрыПолученияДанных - см. НовыеПараметрыПолученияДанныхДокумента - параметры получения данных документа.
// 	
// Возвращаемое значение:
//
Функция ДанныеЭлектронногоДокументаОсновныхТипов(ПараметрыПолученияДанных)
	...
	Результат = Новый Структура;
	Результат.Вставить("ДеревоРазбора", "");
	Результат.Вставить("ДеревоДанные", НовыйЭД);
...
Процедура СформироватьТЗПоНаборуДанныхXDTO(ТЗ, НаборДанных, ДопПараметры = Неопределено) 
	...
	Для Каждого ТекЭлемент Из НаборДанных Цикл
		...
		Если ОписанияРабот <> Неопределено Тогда
...
Функция НовыеДанныеДляСопоставленияТоваров()
	...
	Сопоставление.Вставить("СодержитИзменяемыеДанные", "");
	...
	Сопоставление.Вставить("ТипНоменклатурыФНС"    , );
...
Функция НовыеДанныеДляСопоставленияТоваровПоДаннымНоменклатуры(Знач Идентификатор, Знач Наименование, Знач Характеристика, Знач ЕдиницаИзмерения)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) Тогда
		Идентификатор = СопоставлениеНоменклатурыКонтрагентов.ИдентификаторТовараПоДаннымНоменклатуры(Наименование, Характеристика, ЕдиницаИзмерения);
	КонецЕсли;
...
Функция СоответствиеТитулов()
	...
	СоответствиеТитулов.Вставить(Форматы.ФНС.ПередачаТоваров.ИнформацияПродавца, Форматы.ФНС.ПередачаТоваров.ИнформацияПокупателя);
	СоответствиеТитулов.Вставить(Форматы.ФНС.ПередачаРабот.ИнформацияПродавца, Форматы.ФНС.ПередачаРабот.ИнформацияПокупателя);
	СоответствиеТитулов.Вставить(Форматы.ФНС.ПриемкаСдачаРабот.ИнформацияПродавца, Форматы.ФНС.ПриемкаСдачаРабот.ИнформацияПокупателя);
...
Функция ЗаполнитьДанныеДополнительныхПолейШапки(ДополнительныеДанные, ПараметрыЗаполнения)
	...
	Если МассивНастроек.Количество() = ДанныеДерева.Количество() Тогда
		ПутьКДопДанным = ПараметрыЗаполнения.Раздел.ПутьКРеквизитуДереваДопДанные;
		ДеревоДанных = ПараметрыЗаполнения.ДеревоДанных;
		Если ЭлектронноеВзаимодействие.СуществуетРеквизитВДереве(ДеревоДанных, ПутьКДопДанным + ".НомерСтроки") Тогда
			...
			Для Каждого Реквизит Из ДанныеДерева Цикл
				...
				ДеревоЭлектронногоДокументаБЭД.ДобавитьЗаписьВТаблицуДерева(ДеревоДанных, СтруктураДанныхДерева, ПутьКДопДанным);
			КонецЦикла;
		Иначе
			СтрокаДопДанных = ДеревоДанных.Строки.Найти(ПутьКДопДанным, "ПолныйПуть", Истина);
