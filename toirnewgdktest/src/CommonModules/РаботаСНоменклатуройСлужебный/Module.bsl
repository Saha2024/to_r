...
//
// Возвращаемое значение:
//  Структура - см. ИнициализацияНастройкиВыгрузкиНоменклатуры
//
Функция НастройкаВыгрузкиНоменклатуры(Организация) Экспорт 
	...
	Если ТипЗнч(НастройкиВыгрузки) = Тип("Соответствие") Тогда
		Настройка = НастройкиВыгрузки.Получить(Организация);
	КонецЕсли;
	
	Если ТипЗнч(Настройка) <> Тип("Структура") Тогда
		Настройка = ИнициализацияНастройкиВыгрузкиНоменклатуры(Организация);
	КонецЕсли;
...
Функция ОбновитьПараметрыНастройкиВыгрузки(Организация, ОбновляемыеПараметры) Экспорт 
	...
	ПустаяНастройка = ИнициализацияНастройкиВыгрузкиНоменклатуры(Организация);
	Настройка       = НастройкиВыгрузки.Получить(Организация);
	Если ТипЗнч(Настройка) <> Тип("Структура") Тогда
		Настройка = ПустаяНастройка;
	Иначе 
		Для каждого КлючИЗначение Из ПустаяНастройка Цикл
			Если НЕ Настройка.Свойство(КлючИЗначение.Ключ) Тогда
				Настройка.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(Настройка, ОбновляемыеПараметры);
	НастройкиВыгрузки.Вставить(Организация, Настройка);

	Константы.НастройкиВыгрузкиНоменклатуры.Установить(Новый ХранилищеЗначения(НастройкиВыгрузки, Новый СжатиеДанных(9)));
...
Функция ВыгрузитьДанныеНоменклатуры(НастройкаВыгрузки) Экспорт 
	...
	ОбновляемыеПараметры.Вставить("ДатаПредставление",      НСтр("ru = 'выгружается'"));
	ОбновляемыеПараметры.Вставить("ВыгруженоПредставление", НСтр("ru = 'Выгружается...'"));
	ОбновляемыеПараметры.Вставить("ДатаВыгрузки",           Дата(1, 1, 1));
	ОбновитьПараметрыНастройкиВыгрузки(Организация, ОбновляемыеПараметры);
...
//
Функция ИнициализацияНастройкиВыгрузкиНоменклатуры(Организация)
	...
	СтруктураНастройки.Вставить("party_id"                       "");
...
Процедура ПроверитьЗаполнениеНоменклатуры(Запрос, Настройка, ПодготовитьЗапрос = Истина, РезультатПроверки = Неопределено)
	...
	Если Настройка.ВыгружатьВНациональныйКаталог = Истина Тогда
		...
		Если Настройка.ПолучитьШтрихкоды = Истина Тогда
...
Функция ВыгрузитьНоменклатуру(Запрос, Параметры)
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Параметры.party_id) Тогда
		ПараметрПартнера = Новый Структура("partnerCode, property, value");
		ПараметрПартнера.partnerCode = "crpt";
		ПараметрПартнера.property    = "party_id";
		ПараметрПартнера.value       = Параметры.party_id;
		ПараметрыПартнера.Добавить(ПараметрПартнера);
	КонецЕсли;
	...
	Если Параметры.ПолучитьШтрихкоды = Истина Тогда
		ПолучитьШтрихкоды = Новый Структура("partnerCode, property, value", "crpt", "generateGTINs", "1");
		Отправитель.Вставить("partnerOptions", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПолучитьШтрихкоды));
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаТаблицыТовары Из ОбходРезультатов.ТоварыСлужебный Цикл
			...
			// Штрихкоды
			Если ЗначениеЗаполнено(ОбходРезультатов.Штрихкоды) Тогда
				ДанныеПакета = Новый СписокЗначений;
...
Функция ВыполнитьДействиеВПовторе(Действие, ПараметрыДействия, ПараметрыПовтора)
	...
	Если Действие = "ЗапросКСервису" Тогда
		Результат = ЗапросКСервису(ПараметрыДействия, ВыполнятьПовтор);
	Иначе 
...
Функция ЗапросКСервису(ПараметрыКоманды, ВыполнятьПовтор)
	...
	Если ПараметрыКоманды.Аутентификация = Истина Тогда
		ЗаголовокАутентификации = ЗаголовокАутентификации(Соединение, ПараметрыКоманды, ВыполнятьПовтор);
		Если Не ВыполнятьПовтор Тогда
	...
	РезультатДействия = РезультатДействияЗапросКСервису(Соединение, HTTPЗапрос, ПараметрыКоманды, ВыполнятьПовтор);
...
Функция ЗаголовокАутентификации(Соединение, ПараметрыКоманды, ВыполнятьПовтор)
	
	Если НРег(ПараметрыКоманды.ТипАутентификации) = "bearer" Тогда
		Результат = ТокенАутентификацииВСервисе(Соединение, ВыполнятьПовтор);
		Если Результат.Результат <> Неопределено Тогда
...
Функция ТокенАутентификацииВСервисе(Соединение, ВыполнятьПовтор)
	...
	Если КлючДоступа = Неопределено Или Не СрокТокенаОбновленияАктуален(КлючДоступа) Тогда
		Результат = НовыйТокенАутентификацииВСервисе(Соединение, ВыполнятьПовтор);
	ИначеЕсли Не СрокТокенаДоступаАктуален(КлючДоступа) Тогда  
		Результат = ОбновленныйТокенАутентификацииВСервисе(КлючДоступа, Соединение, ВыполнятьПовтор); 
	Иначе 
		Результат = НовыйРезультатДействия();
...
Функция НовыйТокенАутентификацииВСервисе(Соединение, ВыполнятьПовтор)
	...
	Если ТикетАутентификацииИТС.Результат = Неопределено Тогда
		...
		Иначе
			Возврат НовыйТокенАутентификацииПоЛогинуИПаролю(Соединение, ВыполнятьПовтор);
		КонецЕсли;
	КонецЕсли;
	...
	Возврат ТокенАутентификацииСервиса(Соединение, ПараметрыКоманды, ВыполнятьПовтор);
	
КонецФункции

Функция НовыйТокенАутентификацииПоЛогинуИПаролю(Соединение, ВыполнятьПовтор)
...
Функция ОбновленныйТокенАутентификацииВСервисе(КлючДоступа, Соединение, ВыполнятьПовтор)
...
Функция ТокенАутентификацииСервиса(Соединение, ПараметрыКоманды, ВыполнятьПовтор)
	...
	ОтветСервиса = РезультатДействияЗапросКСервису(Соединение, Запрос, ПараметрыКоманды, ВыполнятьПовтор);
	Если ОтветСервиса.Результат = Неопределено Или Не ВыполнятьПовтор Тогда
...
Функция РезультатДействияЗапросКСервису(Соединение, Запрос, ПараметрыКоманды, ВыполнятьПовтор)
	...
	ИначеЕсли ЭтоHTTPУспех(Ответ) Тогда
		Результат.Результат = Ответ;
	Иначе
		ВыполнятьПовтор = ДопустимПовторныйЗапрос(Ответ, ПараметрыКоманды);
		
		ПараметрыКоманды.Вставить("БлокироватьСообщенияОбОшибках", ВыполнятьПовтор);
		Отказ = Истина;
		ПараметрыОшибки = Неопределено;
		ОбработатьОшибкуСервиса(ПараметрыКоманды, Ответ.КодСостояния, ПараметрыОшибки, Отказ);
	КонецЕсли;
