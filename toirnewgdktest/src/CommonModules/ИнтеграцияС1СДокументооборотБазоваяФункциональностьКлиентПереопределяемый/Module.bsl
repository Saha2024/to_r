...
Процедура ОткрытьПанельАдминистрированияБИД(ПараметрыВыполненияКоманды, СтандартнаяОбработка) Экспорт
	...
	Если НЕ торо_СЛКСервер.СерияЗащитыИнициализирована("28DD", Истина) Тогда 
		СтандартнаяОбработка = Ложь;
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Интеграция доступна только для версии КОРП.'"));
		НаборИмен = Новый Массив;
		НаборИмен.Добавить("ИспользоватьИнтеграциюС1СДокументооборот"); 
		НаборИмен.Добавить("ИспользоватьИнтеграциюС1СДокументооборот3");
