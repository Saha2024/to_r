...
Процедура ДополнительныеРеквизитыОбработкаПроверкиЗаполнения(Источник, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("СправочникСсылка.НаборыДополнительныхРеквизитовИСведений")
		// ТОИР++
		ИЛИ ТипЗнч(Источник.Ссылка) = Тип("СправочникСсылка.торо_ТиповыеОР")
		ИЛИ ТипЗнч(Источник.Ссылка) = Тип("СправочникСсылка.торо_НаправленияОбъектовРемонтныхРабот")
		// ТОИР--
		Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПеренестиЗначенияИзРеквизитовФормыВОбъект(Форма, Объект = Неопределено, ПередЗаписью = Ложь) Экспорт
	...
	Если Форма.Свойства_ОтображатьЗначенияВТаблице Тогда
		ТаблицаСвойств = Форма.Свойства_ЗначенияДополнительныхРеквизитов;
	КонецЕсли;
	// Доп. реквизиты в таблице--
	...
	Для каждого Строка Из ТаблицаСвойств Цикл
		...
		// Доп. реквизиты в таблице++
		Если  Форма.Свойства_ОтображатьЗначенияВТаблице Тогда
			ИспользоватьСтрокуВВидеСсылки = Ложь;
		// Доп. реквизиты в таблице--
		Иначе	
...
//
Функция ЗначенияСвойств(ДополнительныеСвойстваОбъекта, Наборы, ЭтоДополнительноеСведение, СкрыватьУдаленные = Истина) Экспорт
	...
	Если ДополнительныеСвойстваОбъекта.Количество() = 0 Тогда
		...
		СвойстваНеНайдены = ДополнительныеРеквизитыИСведенияНеНайдены(Наборы, ЭтоДополнительноеСведение,,СкрыватьУдаленные);
		// ТОИР--
	...
	// ТОИР++
	Если НЕ СкрыватьУдаленные Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "НЕ СвойстваНаборов.ПометкаУдаления",	"ИСТИНА");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "НЕ Свойства.ПометкаУдаления",	"ИСТИНА");
	КонецЕсли;
	// ТОИР--
...
//
Функция ДополнительныеРеквизитыИСведенияНеНайдены(Наборы, ЭтоДополнительноеСведение, ОтложеннаяИнициализация = Ложь, СкрыватьУдаленные = Истина) Экспорт
	...
	// ТОИР++
	Если НЕ СкрыватьУдаленные Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И НЕ СвойстваНаборов.ПометкаУдаления",	"");
	КонецЕсли;
	// ТОИР--
