...
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	// ТОИР++
   Контекст.Вставить("ИмяРеквизитаНаборСвойств",   "");
	Контекст.Вставить("УсловноеОформлениеЗаполнятьОбязательно",   Истина);
	// ТОИР--
	...
	СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов, ОписаниеОбъекта);
	...
	Если Не Контекст.ПроизвольныйОбъект
		...
		ЗаполнитьДополнительныеРеквизитыВФорме(Форма, ОписаниеОбъекта, , Контекст.СкрытьУдаленные, Контекст.ИмяРеквизитаНаборСвойств);
		// Доп. реквизиты в таблице++
		торо_УправлениеСвойствами.ДобавитьНаФормуТаблицуЗначенийДополнительныхРеквизитов(Форма, Контекст);
		// Доп. реквизиты в таблице--
	КонецЕсли;
...
//
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект, ИмяРеквизитаНаборСвойств = "") Экспорт
	...
	Если ТипЗнч(Структура.Свойства_ИспользоватьСвойства) = Тип("Булево")
      ...
      ЗаполнитьДополнительныеРеквизитыВФорме(Форма, ТекущийОбъект,,, ИмяРеквизитаНаборСвойств);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Форма, Отказ, ПроверяемыеРеквизиты, Объект = Неопределено) Экспорт
	...
	Если Форма.Свойства_ОтображатьЗначенияВТаблице Тогда
		торо_УправлениеСвойствами.ОбработкаПроверкиЗаполненияВТаблице(Форма, Отказ, ПроверяемыеРеквизиты, Объект);		
	// Доп. реквизиты в таблице--
	Иначе
...
//  ИмяРеквизитаНаборСвойств - Строка - название реквизита, являющегося ссылкой на набор доп. реквизитов.
//
Процедура ОбновитьЭлементыДополнительныхРеквизитов(Форма, Объект = Неопределено, СкрытьУдаленные = Неопределено, ИмяРеквизитаНаборСвойств = "") Экспорт
	
	УправлениеСвойствамиСлужебный.ПеренестиЗначенияИзРеквизитовФормыВОбъект(Форма, Объект);
	
	ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект, , СкрытьУдаленные, ИмяРеквизитаНаборСвойств);
...
//  ИмяРеквизитаНаборСвойств - Строка - название реквизита, являющегося ссылкой на набор доп. реквизитов.
//
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено, ИмяРеквизитаНаборСвойств = "") Экспорт
	...
	// Доп. реквизиты в таблице++
	ОтображатьЗначенияВТаблице = Форма.Свойства_ОтображатьЗначенияВТаблице;
	// Доп. реквизиты в таблице--
   ...
   УправлениеСвойствамиСлужебный.ЗаполнитьНаборыСДополнительнымиРеквизитами(
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		// Доп. реквизиты в таблице++
		Если ОтображатьЗначенияВТаблице Тогда
			торо_УправлениеСвойствами.ЗаполнитьДополнительныеРеквизитыВФорме_ДобавитьСтрокуТаблицы(Форма, ОписаниеСвойства);
		Иначе
		// Доп. реквизиты в таблице--	
	...
	// Доп. реквизиты в таблице++
	Если ОтображатьЗначенияВТаблице Тогда
		торо_УправлениеСвойствами.ЗаполнитьДополнительныеРеквизитыВФорме_ВычислитьУсловияДопРеквизитаВТаблице(Форма, ОписаниеОбъекта);
	Иначе
	// Доп. реквизиты в таблице--	
...
Процедура УдалитьСтарыеРеквизитыИЭлементы(Форма) Экспорт
	...
	// Доп. реквизиты в таблице++
	Форма.Свойства_ЗначенияДополнительныхРеквизитов.Очистить();
	// Доп. реквизиты в таблице--
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов, Объект = Неопределено)
   ...
   Если Объект = Неопределено Тогда
	...
	// Доп. реквизиты в таблице++
	РеквизитОтображатьЗначенияВТаблице = Новый РеквизитФормы("Свойства_ОтображатьЗначенияВТаблице", Новый ОписаниеТипов("Булево"));
	Реквизиты.Добавить(РеквизитОтображатьЗначенияВТаблице);
	// Доп. реквизиты в таблице--
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			// Доп. реквизиты в таблице++	
			торо_УправлениеСвойствами.ДобавитьОписаниеТаблицыЗначенийДополнительныхРеквизитов(Реквизиты);
			// Доп. реквизиты в таблице--		
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		// Доп. реквизиты в таблице++
		ЭтоСправочникОР = ТипЗнч(ОписаниеОбъекта.Ссылка) = Тип("СправочникСсылка.торо_ОбъектыРемонта")
			ИЛИ ТипЗнч(ОписаниеОбъекта.Ссылка) = Тип("СправочникСсылка.торо_ТиповыеОР");
		
		Форма.Свойства_ОтображатьЗначенияВТаблице = ЭтоСправочникОР;
		// Доп. реквизиты в таблице--
...
Функция РезультатВычисленияУсловия(Форма, ОписаниеОбъекта, Параметры) Экспорт
...
Функция КлючНаборовСвойств(Наборы)
	...
	// ТОИР++
	ЭтоСправочникОбъектыРемонта = (Наборы.НайтиПоЗначению(НаборСвойствПоИмени("Справочник_торо_ТиповыеОР_Общие"))<>Неопределено
										ИЛИ Наборы.НайтиПоЗначению(НаборСвойствПоИмени("Справочник_торо_ОбъектыРемонта_общие"))<>Неопределено);
	// ТОИР--
	...
	Если НЕ ЭтоСправочникОбъектыРемонта Тогда  // для сохранения настроек в ОР для всего справочника
