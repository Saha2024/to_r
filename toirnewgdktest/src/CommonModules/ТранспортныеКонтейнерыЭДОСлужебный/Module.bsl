...
Процедура ПодготовитьДанныеПоТранспортнойИнформации(ОписаниеОбъектов, ДанныеФайлов, ДеревоТранспортнойИнформации)
	...
	Для Каждого ОписаниеОбъекта Из ОписаниеОбъектов Цикл
		...
		СтруктураФайловЭД = ДанныеФайлов[ОписаниеОбъекта.ИдентификаторСтроки];
		...
		Иначе
			КодРегламента = ТипРегламентаСтрокой(ОписаниеОбъекта.ТипРегламента);
			КодТранзакции = ПолучитьКодТранзакции(ОписаниеОбъекта.ТипЭлементаРегламента, КодРегламента,
				ОписаниеОбъекта.ТипДокумента, ОписаниеОбъекта.ЭтоОтветнаяПодпись);
		КонецЕсли;
...
Функция ПолучитьКодТранзакции(ТипЭлементаРегламента, КодРегламента, ТипДокумента, ОтветнаяПодпись)
...
Функция ТипРегламентаСтрокой(ТипРегламента)
	
	Таблица = ТаблицаТиповРегламентов();
	
	СтрокаТаблицы = Таблица.Найти(ТипРегламента, "ТипРегламента");
...
Функция ТипРегламентаИзСтроки(КодРегламента) Экспорт
	
	Таблица = ТаблицаТиповРегламентов();
	
	СтрокаТаблицы = Таблица.Найти(КодРегламента, "КодРегламента");
...
Процедура СохранитьПодписи(ОписаниеОбъекта,
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		ИмяФайлаПодписи = РаботаСФайламиБЭД.ДопустимоеИмяФайла(ИмяФайлаПодписи, Истина);
