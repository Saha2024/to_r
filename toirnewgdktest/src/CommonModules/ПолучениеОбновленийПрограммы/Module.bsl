...
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Не НастройкиЗагрузкиИсправлений.ОтключитьНапоминания
		...
		Если ДатаИнформирования <= ТекущаяДатаСеанса() Тогда
			Параметры.Вставить("ПоказатьОповещениеОВключенииАвтоматическойЗагрузкиИсправлений", Истина);
		КонецЕсли;
...
Процедура ИнтернетПоддержкаИСервисы_ПриСозданииНаСервере(Форма) Экспорт
	...
	ПараметрыДоступа = ПараметрыДоступаКОбновлениюПрограммы();
	ДоступенПросмотрИнформацииОбОбновлениях = ПараметрыДоступа.ДоступнаПодсистема
		И Не ПараметрыДоступа.ЭтоВебПодключение;
	...
	Элементы.ГруппаОбновлениеПрограммы_Настройки.Видимость  = ПараметрыДоступа.ДоступнаПодсистема;
...
//
// Возвращаемое значение:
//  Неопределено - возвращается если обновление программы не доступно, длительная операция отменена или возникла
//    модульная ошибка.
//  Структура - возвращается если запущенное задание имеет статус Выполняется или Выполнено:
//    * Идентификатор - УникальныйИдентификатор - идентификатор запущенной длительной операции.
//    * АдресРезультата - Строка - адрес временного хранилища с результатом работы проверки.
//    * Выполнено - Булево - Истина, если длительная операция выполнено.
//
...
Процедура ПроверитьНаличиеОбновленийПриЗапуске(Параметры)
	...
	Если Не ПроверкаНаличияОбновления.ПроверкаЗапущена Тогда
		Параметры.Вставить("ПроверкаНаличияОбновления", ПроверкаНаличияОбновления);
		Возврат;
	КонецЕсли;
	
	Если НастройкиОбновления.РежимАвтоматическойПроверкиНаличияОбновленийПрограммы = 2 Тогда
		// Записать дату последней проверки.
		Параметры.ПолучениеОбновленийПрограммы.Вставить("ДатаПоследнейПроверки", ДатаПроверки);
...
Функция ЗагрузитьИУстановитьОбновленияВФоновомРежиме(Параметры) Экспорт
	...
	Если Контекст.ОбновленияКонфигурации.Количество() = 0
		...
		Если РезультатУстановки.Ошибка Тогда
			...
			Контекст.Вставить("ИнформацияОбОшибке"   , РезультатУстановки.ИнформацияОбОшибке);
			Контекст.Вставить("СообщениеТехподдержке", РезультатУстановки.СообщениеТехподдержке);
...
Функция ИнформацияОДоступномОбновленииСлужебная(
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		КонецЕсли;
		Если ДополнительныеПараметры.Свойство("ТолькоИсправления") Тогда
			ТолькоИсправления = ДополнительныеПараметры.ТолькоИсправления;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ТолькоИсправления Тогда
