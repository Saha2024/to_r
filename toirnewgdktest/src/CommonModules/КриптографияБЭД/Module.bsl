...
Функция ИННСубъектаСертификата(СертификатКриптографии) Экспорт
	...
	СвойстваСубъекта = СвойстваСубъектаСертификата(СертификатКриптографии);
	Возврат СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(СвойстваСубъекта.ИНН, "0");
...
Функция ТекстШтампаЭП(ДанныеДляФормирования, Подписи, ТекстШтампа)
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY%", Формат(НачальноеСмещенияПоВертикали + 70, "ЧРД=.; ЧГ=0"));
		...
		Если ОрганизацияПодписант.Количество() > 0 Тогда
			...
			СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ОрганизацияПодписант%", ОрганизацияПодписант[0]);
			ТекстЭП.ДобавитьСтроку(СтрокаЭП);
			...
			Если ОрганизацияПодписант.Количество() > 1 Тогда
			    Для Индекс = 1 По ОрганизацияПодписант.Количество() - 1 Цикл
					...
					СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ОрганизацияПодписант%", ОрганизацияПодписант[Индекс]);
					ТекстЭП.ДобавитьСтроку(СтрокаЭП);
				КонецЦикла;
			КонецЕсли;
		...
		Если Счетчик <> КоличествоПодписей Тогда
			...
			СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY1%", Формат(КонечноеСмещенияПоВертикали + 95, "ЧРД=.; ЧГ=0"));
			СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY2%", Формат(КонечноеСмещенияПоВертикали + 95, "ЧРД=.; ЧГ=0"));
			ТекстЭП.ДобавитьСтроку(СтрокаЭП);
	...
	// Подвал.	
	ВысотаШтампаБезПодвала = 50 + 40*КоличествоПодписей + ИтоговоеСмещениеПоКоличествуСтрок;
	...
	СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ИдентификаторДокумента%", ДанныеДляФормирования.ИдентификаторДокумента);
	ТекстЭП.ДобавитьСтроку(СтрокаЭП);
...
Функция ТекстШтампаПЭП(ДанныеДляФормирования, Подписи, ТекстШтампа)
	...
	ВысотаШтампаБезПодвалы = 50 + 40*КоличествоПодписей;
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		СтрокаЭП = МакетЭП.ПолучитьСтроку(12);
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY%", Формат(СмещениеПоВертикали + 70, "ЧРД=.; ЧГ=0"));
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ЦветТекста%", ЦветТекста);
		ТекстЭП.ДобавитьСтроку(СтрокаЭП);
		
		СтрокаЭП = МакетЭП.ПолучитьСтроку(13);
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%Организация%", ПараметрыДляЗаполненияШтампа.Организация);
		ТекстЭП.ДобавитьСтроку(СтрокаЭП);
		
		СтрокаЭП = МакетЭП.ПолучитьСтроку(14);
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ФИОПодписанта%", ПараметрыДляЗаполненияШтампа.ФИОПодписанта);
		ТекстЭП.ДобавитьСтроку(СтрокаЭП);
		...
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY%", Формат(СмещениеПоВертикали + 70, "ЧРД=.; ЧГ=0"));
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ЦветТекста%", ЦветТекста);
		СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ИмяПользователя%", ПараметрыДляЗаполненияШтампа.ИмяПользователя);
		ТекстЭП.ДобавитьСтроку(СтрокаЭП);
		...
		Если Счетчик <> КоличествоПодписей Тогда
			...
			СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY1%", Формат(СмещениеПоВертикали + 95, "ЧРД=.; ЧГ=0"));
			СтрокаЭП = СтрЗаменить(СтрокаЭП, "%ПозицияY2%", Формат(СмещениеПоВертикали + 95, "ЧРД=.; ЧГ=0"));
			ТекстЭП.ДобавитьСтроку(СтрокаЭП);
