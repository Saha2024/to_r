...
Процедура ПослеФормирования(ФормаОтчета, ОтчетСформирован) Экспорт
	...
	Если ОтчетСформирован
		И (ФормаОтчета.ИмяФормы = "Отчет.торо_ПланФактныйАнализППР.Форма"
			Или ФормаОтчета.ИмяФормы = "Отчет.торо_АнализДефектовИПотерь.Форма"
				И ФормаОтчета.КлючТекущегоВарианта = "АнализДефектовИПотерьДиаграмма") Тогда
		торо_ОтчетыСервер.ОбработатьТабличныйДокументПослеФормированияОтчета(ФормаОтчета.ИмяФормы, ФормаОтчета.ОтчетТабличныйДокумент);
	КонецЕсли;
...
Процедура ОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ФормаОтчета.ИмяФормы = "Отчет.ТоварыСхемыОбеспечения.Форма" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ЗначениеНужногоПоляВСтроке = Неопределено;
		МассивИменТекущихПолей = Новый Массив();  
		МассивИменТекущихПолей.Добавить("Номенклатура");
		
		ЗначениеНужногоПоляВСтроке = торо_ОтчетыСервер.ПолучитьЗначениеВыбранногоПоляНаСервере (ФормаОтчета.ОтчетДанныеРасшифровки, Расшифровка, МассивИменТекущихПолей);
		
		Если НЕ ЗначениеНужногоПоляВСтроке = Неопределено Тогда
			ПоказатьЗначение(, ЗначениеНужногоПоляВСтроке);
		КонецЕсли;
