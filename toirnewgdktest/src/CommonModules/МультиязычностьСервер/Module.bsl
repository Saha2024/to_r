...
Процедура ПередЗаписьюНаСервере(ТекущийОбъект) Экспорт
	...
	Если МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Тогда
		...
		Для Каждого Реквизит Из ИменаЛокализуемыхРеквизитов Цикл
			...
			// ТОИР++
			// У групп могут быть не все реквизиты.
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Знач Параметры, СтандартнаяОбработка, ОбъектМетаданных) Экспорт
	...
	// ТОИР++
	Запрос.Текст = Запрос.Текст + " И &ДополнительныеУсловияОтбора";
	торо_ОбщегоНазначения.СформироватьСтрокуОтборовПоПараметрамПолученияДанныхВыбора(Запрос, Параметры);
	// ТОИР--
...
Процедура ИзменениеТекстаЗапросаСпискаДляТекущегоЯзыка(Форма, ИмяСписка = "Список",
	// ТОИР++
				...
				Если ТипЗнч(ИсточникЗапроса.Источник) <> Тип("ВложенныйЗапросСхемыЗапроса")
					...
					Для каждого ОписаниеРеквизита Из ЛокализуемыеРеквизиты Цикл
						...
						Для Индекс = 0 По ОператорЗапроса.ВыбираемыеПоля.Количество() - 1 Цикл
							...
							// ТОИР++
							// Проверяем, что название реквизита совпадает полностью, а не только частично.
							// Например, если искали "Наименование", а нашли "НаименованиеПолное" - то это не тот реквизит.
							СледующийСимвол = Сред(ВыбираемоеПоле, Позиция+СтрДлина(ПолноеИмя), 1);
							Если СледующийСимвол <> "" И СледующийСимвол <> Символы.ПС 
								И СледующийСимвол <> " " И СледующийСимвол <> "," Тогда
								Продолжить;
							КонецЕсли;
							// ТОИР--
							...
							// ТОИР++
							СтарыйПсевдонимПоля = ПакетЗапроса.Колонки.Получить(Индекс).Псевдоним;
							// ТОИР--
							...
							//КонецЕсли;
							Если НЕ ЗначениеЗаполнено(СтарыйПсевдонимПоля) Тогда
								Если ПакетЗапроса.Колонки.Найти(ИмяОсновногоРеквизита) = Неопределено Тогда
									ПакетЗапроса.Колонки.Получить(Индекс).Псевдоним = ИмяОсновногоРеквизита;
								КонецЕсли;
							Иначе
								ПакетЗапроса.Колонки.Получить(Индекс).Псевдоним = СтарыйПсевдонимПоля;
							КонецЕсли;
							// ТОИР--
...
Функция ЛокализуемыеРеквизитыОбъектаДляТекущегоЯзыка(ОбъектМетаданных, Язык = Неопределено)
	...
	// ТОИР++
	// Для регистров сведений проверим дополнительно ресурсы. Используется в запросах рабочих мест.
	Если ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных) Тогда
		ОбъектМетаданныхРесурсы = ОбъектМетаданных.Ресурсы;
		Для Каждого Реквизит Из ОбъектМетаданныхРесурсы Цикл
			СписокРеквизитовОбъекта.Вставить(Реквизит.Имя, Реквизит);
		КонецЦикла;		
	КонецЕсли;
	// ТОИР--
