...
Процедура ВыгрузитьНоменклатуру() Экспорт
	...
	Если ТипЗнч(НастройкиВыгрузкиНоменклатуры) <> Тип("Соответствие") Тогда
		Возврат
	КонецЕсли;
	Для каждого ОписаниеНастройкиВыгрузки Из НастройкиВыгрузкиНоменклатуры Цикл
	...
	Для Каждого ЧастьСтроки Из СтрокиСоСсылками Цикл
		...
		Если ЧастьСтроки.Пометка Тогда
			...
			Если ЦветВыделения <> Неопределено Тогда
				МассивСтроки.Добавить(Новый ФорматированнаяСтрока(ЧастьСтроки.Значение, Новый Шрифт(,,Истина), ЦветВыделения));
			Иначе
