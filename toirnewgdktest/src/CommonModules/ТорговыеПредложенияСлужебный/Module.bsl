...
Функция ВыполнитьЗапрос(ЗапросСервиса, Отказ, Ошибка = Неопределено) Экспорт
	...
	Исключение
		...
		ПодробноеПредставлениеОшибки = ТекстОшибки + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
//
Функция ПолучитьОрганизацииОпубликованныхТорговыхПредложений() Экспорт
...
//  КратностьПоУмолчанию 	- Булево - Если Истина то получить записи с кратностью = 1, иначе > 1.
// 
// Возвращаемое значение:
//  Массив - Данные кратности упаковок публикуемых торговых предложений, см.НовыеДанныеКратностиУпаковок.
//
Функция ДанныеКратностиУпаковокПубликуемыхТорговыхПредложений(ПрайсЛист, КратностьПоУмолчанию = Ложь) Экспорт
	...
	Возврат РегистрыСведений.ТорговыеПредложенияТорговойПлощадки.
		ДанныеКратностиУпаковокПубликуемыхТорговыхПредложений(ПрайсЛист, КратностьПоУмолчанию);
...
Процедура СоздатьКорзинуИЗаполнитьКоманды(Организация, АдресКорзины, АдресКомандыДобавитьТовар) Экспорт
...
Процедура ПеречитатьКорзинуНаФормахПоискаПоОтборуИПоТоварам(
	АдресКорзины, АдресКомандыДобавитьТовар, ДанныеКорзины, ТаблицаТоваров) Экспорт
...
Процедура ДополнитьДаннымиКатегорий(Результат, ПараметрыЗапроса)
	...
	ПараметрыЗапросаКатегорий = ОписаниеПараметровДереваКатегорий();
...
Процедура ЗаполнитьПараметрыДереваКатегорийПоПараметрамПоискаПредложений(ПараметрыЗапросаКатегорий, ПараметрыПоиска)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыЗапросаКатегорий, ПараметрыПоиска, "ВНаличии, СоСкидкой");
...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|	ТорговыеПредложенияТорговойПлощадки.КратностьУпаковки КАК КратностьУпаковки,
	...
	|	ТорговыеПредложенияТорговойПлощадки.СостояниеПубликацииТорговыхПредложений КАК
	...
	|			И (НЕ ТоварыСлужебный.Отключено)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция НовыеДанныеКратностиУпаковок() Экспорт
	...
	ДанныеКратностиУпаковок.Вставить("Номенклатура", МетаданныеИзмерений.Номенклатура.Тип);
	ДанныеКратностиУпаковок.Вставить("Характеристика", МетаданныеИзмерений.Характеристика.Тип);
	ДанныеКратностиУпаковок.Вставить("Упаковка", МетаданныеИзмерений.Упаковка.Тип);
...
Процедура ПроверитьРеквизитыКонтрагентов(ТаблицаКонтрагентов, ПрайсЛист, Отказ)
	...
	СписокОшибок = СтрСоединить(Ошибки, "," + Символы.ПС);
...
Процедура ВыполнитьОбновлениеТорговыхПредложенийТорговойПлощадки(Знач ПараметрыПроцедуры, АдресРезультата, КоличествоЗаписей = 0)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Обновление торговых предложений торговой площадки.'");
...
Процедура ПроверитьЗаписиТорговыеПредложенияТорговойПлощадки(Отбор, АдресРезультата = Неопределено) Экспорт
	...
	ТаблицыДляПолучения = 
		"ТоварыСлужебный, ПрайсЛистыСлужебный, Контакты, СвойстваСлужебный, Штрихкоды, Контрагенты, Изображения";
	...
	Если ВариантыПубликацииСкидок.Найти(Перечисления.ВариантыПубликацииСкидок.ВидыЦен) <> Неопределено Тогда
		...
		ТаблицыДляПолучения = ТаблицыДляПолучения + ", СкидкиЗаОбъемПродажВидыЦен, СкидкиПроцентСуммаВидыЦен";
	КонецЕсли;
	...
	Если ВариантыПубликацииСкидок.Найти(Перечисления.ВариантыПубликацииСкидок.ВидыСкидок) <> Неопределено Тогда
		...
		ТаблицыДляПолучения = ТаблицыДляПолучения + ", СкидкиЗаОбъемПродаж, СкидкиПроцентСумма";
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицаТоваров Из ТаблицаТоваров Цикл
		...
		ДанныеПрайсЛиста.Вставить("ТаблицаИзображения"        , ТаблицаИзображения);
...
Функция ДанныеПозицииТорговогоПредложенияИзменены(ДанныеПозиции)
	...
	СтрокиИзображений = ДанныеПозиции.ТаблицаИзображения.НайтиСтроки(СтруктураПоиска);
	...
	Возврат Не ДанныеПозиции.ХешНовый = СтрокаТаблицаТоваров.ХешПредложения
...
Процедура ПроверитьРеквизитыМатрицы(Таблица, ПараметрыОтбора, ПроверяемыеРеквизиты, ТаблицаОшибок)
	...
	Для Каждого СтрокаМатрицы Из СтрокиМатрицы Цикл
		ПроверитьРеквизитыСтроки(СтрокаМатрицы, ПроверяемыеРеквизиты, ТаблицаОшибок);
	КонецЦикла;
КонецПроцедуры
...
Процедура ОпределитьВладельцевНезаполненныхРеквизитов(
	ПроверяемыйРеквизит, СтрокаТорговогоПредложения, СсылкаНаОбъект, ПутьКДанным, 
	...
	ИначеЕсли ВРег(ПроверяемыйРеквизит) = ВРег("РегистрацияОрганизации") Тогда
		...
		Возврат;
	ИначеЕсли ВРег(ПроверяемыйРеквизит) = ВРег("ПрайсЛисты.АдресЭлектроннойПочты") Тогда
...
Процедура ВыполнитьСинхронизациюТорговыхПредложенийВФоне(ДлительнаяОперация) Экспорт 
	...
	Для Каждого ВыполняемоеФоновоеЗадание Из ВыполняемыеФоновыеЗадания Цикл
		ИнтеграцияБСПБЭД.ОтменитьВыполнениеЗадания(ВыполняемоеФоновоеЗадание.УникальныйИдентификатор);
	КонецЦикла;
