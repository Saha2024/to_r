...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	ШаблонСообщения = НСтр("ru = 'На %1 у физлица %2 уже введен документ, удостоверяющий личность.'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		Если Не ПустаяСтрока(Запись.Серия) Тогда
			ТекстСообщения = "";
			Отказ = Не СотрудникиКлиентСервер.СерияДокументаУказанаПравильно(Запись.ВидДокумента, Запись.Серия, ТекстСообщения) Или Отказ;
		...
		Если Не ПустаяСтрока(Запись.Номер) Тогда
			ТекстСообщения = "";
			Отказ = Не СотрудникиКлиентСервер.НомерДокументаУказанПравильно(Запись.ВидДокумента, Запись.Номер, ТекстСообщения) Или Отказ;
			Если Не ПустаяСтрока(ТекстСообщения) Тогда
